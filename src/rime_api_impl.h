...
RIME_DEPRECATED void RimeSetup(RimeTraits* traits) {
    ...
    if (RIME_PROVIDED(traits, app_name)) {
        if (RIME_STRUCT_HAS_MEMBER(*traits, traits->min_log_level) && RIME_STRUCT_HAS_MEMBER(*traits, traits->log_dir)) {
...
RIME_DEPRECATED void RimeSetNotificationHandler(RimeNotificationHandler handler, void* context_object) {
    if (handler) {
        Service::instance().SetNotificationHandler([context_object, handler](auto id, auto type, auto value) { handler(context_object, id, type, value); });
    } else {
...
RIME_DEPRECATED void RimeInitialize(RimeTraits* traits) {
    SetupDeployer(traits);
    LoadModules(RIME_PROVIDED(traits, modules) ? traits->modules : kDefaultModules);
...
RIME_DEPRECATED void RimeDeployerInitialize(RimeTraits* traits) {
    SetupDeployer(traits);
    LoadModules(RIME_PROVIDED(traits, modules) ? traits->modules : kDeployerModules);
}
...
RIME_DEPRECATED Bool RimeDeployWorkspace() {
    Deployer& deployer(Service::instance().deployer());
    return Bool(deployer.RunTask("installation_update") && deployer.RunTask("workspace_update") && deployer.RunTask("user_dict_upgrade") && deployer.RunTask("cleanup_trash"));
}
...
RIME_DEPRECATED Bool RimeDeployConfigFile(const char* file_name, const char* version_key) {
    Deployer& deployer(Service::instance().deployer());
...
RIME_DEPRECATED Bool RimeProcessKey(RimeSessionId session_id, int keycode, int mask) {
...
RIME_DEPRECATED Bool RimeGetContext(RimeSessionId session_id, RIME_FLAVORED(RimeContext) * context) {
...
RIME_DEPRECATED Bool RimeGetStatus(RimeSessionId session_id, RIME_FLAVORED(RimeStatus) * status) {
...
RIME_DEPRECATED Bool RimeCandidateListFromIndex(RimeSessionId session_id, RimeCandidateListIterator* iterator, int index) {
    if (!iterator)
        return False;
    an<Session> session(Service::instance().GetSession(session_id));
    if (!session)
...
RIME_DEPRECATED Bool RimeCandidateListBegin(RimeSessionId session_id, RimeCandidateListIterator* iterator) {
...
RIME_DEPRECATED void RimeSetOption(RimeSessionId session_id, const char* option, Bool value) {
    an<Session> session(Service::instance().GetSession(session_id));
    if (!session)
        return;
    Context* ctx = session->context();
...
RIME_DEPRECATED Bool RimeGetOption(RimeSessionId session_id, const char* option) {
    an<Session> session(Service::instance().GetSession(session_id));
    if (!session)
        return False;
    Context* ctx = session->context();
...
RIME_DEPRECATED void RimeSetProperty(RimeSessionId session_id, const char* prop, const char* value) {
    an<Session> session(Service::instance().GetSession(session_id));
    if (!session)
        return;
    Context* ctx = session->context();
...
RIME_DEPRECATED Bool RimeGetProperty(RimeSessionId session_id, const char* prop, char* value, size_t buffer_size) {
    an<Session> session(Service::instance().GetSession(session_id));
    if (!session)
        return False;
    Context* ctx = session->context();
...
RIME_DEPRECATED Bool RimeGetCurrentSchema(RimeSessionId session_id, char* schema_id, size_t buffer_size) {
    an<Session> session(Service::instance().GetSession(session_id));
    if (!session)
        return False;
    Schema* schema = session->schema();
...
RIME_DEPRECATED Bool RimeSelectSchema(RimeSessionId session_id, const char* schema_id) {
    if (!schema_id)
        return False;
    an<Session> session(Service::instance().GetSession(session_id));
    if (!session)
...
static Bool open_config_in_component(const char* config_component, const char* config_id, RimeConfig* config) {
    if (!config_id || !config)
        return False;
...
RIME_DEPRECATED Bool RimeUserConfigOpen(const char* config_id, RimeConfig* config) {
...
RIME_DEPRECATED Bool RimeConfigGetBool(RimeConfig* config, const char* key, Bool* value) {
    if (!config || !key || !value)
        return False;
    Config* c = reinterpret_cast<Config*>(config->ptr);
...
RIME_DEPRECATED Bool RimeConfigGetInt(RimeConfig* config, const char* key, int* value) {
    if (!config || !key || !value)
        return False;
    Config* c = reinterpret_cast<Config*>(config->ptr);
...
RIME_DEPRECATED Bool RimeConfigGetDouble(RimeConfig* config, const char* key, double* value) {
    if (!config || !key || !value)
        return False;
    Config* c = reinterpret_cast<Config*>(config->ptr);
...
RIME_DEPRECATED Bool RimeConfigGetString(RimeConfig* config, const char* key, char* value, size_t buffer_size) {
    if (!config || !key || !value)
        return False;
    Config* c = reinterpret_cast<Config*>(config->ptr);
    if (!c)
...
RIME_DEPRECATED const char* RimeConfigGetCString(RimeConfig* config, const char* key) {
    if (!config || !key)
        return NULL;
    Config* c = reinterpret_cast<Config*>(config->ptr);
    if (!c)
...
RIME_DEPRECATED Bool RimeConfigUpdateSignature(RimeConfig* config, const char* signer) {
    if (!config || !signer)
        return False;
    Config* c = reinterpret_cast<Config*>(config->ptr);
    Deployer& deployer(Service::instance().deployer());
...
struct RimeConfigIteratorImpl {
    ...
    string key;
    string path;
    RimeConfigIteratorImpl<T>(T& container, const string& root_path) : iter(container.begin()), end(container.end()) {
...
RIME_DEPRECATED Bool RimeConfigBeginList(RimeConfigIterator* iterator, RimeConfig* config, const char* key) {
...
RIME_DEPRECATED Bool RimeConfigBeginMap(RimeConfigIterator* iterator, RimeConfig* config, const char* key) {
    if (!iterator || !config || !key)
        return False;
    iterator->list = NULL;
    iterator->map = NULL;
...
RIME_DEPRECATED Bool RimeConfigNext(RimeConfigIterator* iterator) {
    if (!iterator->list && !iterator->map)
        return False;
    if (iterator->list) {
        RimeConfigIteratorImpl<ConfigList>* p = reinterpret_cast<RimeConfigIteratorImpl<ConfigList>*>(iterator->list);
        if (!p)
            return False;
        if (++iterator->index > 0)
            ++p->iter;
    ...
    if (iterator->map) {
        RimeConfigIteratorImpl<ConfigMap>* p = reinterpret_cast<RimeConfigIteratorImpl<ConfigMap>*>(iterator->map);
        if (!p)
            return False;
        if (++iterator->index > 0)
            ++p->iter;
...
RIME_DEPRECATED Bool RimeSimulateKeySequence(RimeSessionId session_id, const char* key_sequence) {
...
RIME_DEPRECATED Bool RimeConfigGetItem(RimeConfig* config, const char* key, RimeConfig* value) {
    if (!config || !key || !value)
        return False;
    Config* c = reinterpret_cast<Config*>(config->ptr);
    if (!c)
...
RIME_DEPRECATED Bool RimeConfigSetItem(RimeConfig* config, const char* key, RimeConfig* value) {
    if (!config || !key)
        return False;
    Config* c = reinterpret_cast<Config*>(config->ptr);
    if (!c)
...
RIME_DEPRECATED Bool RimeConfigSetBool(RimeConfig* config, const char* key, Bool value) {
    if (!config || !key)
        return False;
    Config* c = reinterpret_cast<Config*>(config->ptr);
    if (!c)
...
RIME_DEPRECATED Bool RimeConfigSetInt(RimeConfig* config, const char* key, int value) {
    if (!config || !key)
        return False;
    Config* c = reinterpret_cast<Config*>(config->ptr);
    if (!c)
...
RIME_DEPRECATED Bool RimeConfigSetDouble(RimeConfig* config, const char* key, double value) {
    if (!config || !key)
        return False;
    Config* c = reinterpret_cast<Config*>(config->ptr);
    if (!c)
...
RIME_DEPRECATED Bool RimeConfigSetString(RimeConfig* config, const char* key, const char* value) {
    if (!config || !key || !value)
        return False;
    Config* c = reinterpret_cast<Config*>(config->ptr);
    if (!c)
...
static Bool RimeChangePage(RimeSessionId session_id, Bool backward) {
    ...
    size_t index = backward ? (current_index <= page_size ? 0 : current_index - page_size) : (current_index + page_size);
    DLOG(INFO) << "current selection: " << current_index << ", flipping " << (backward ? "backward" : "forward") << ", new selection " << index;
...
static Bool RimeHighlightCandidateOnCurrentPage(RimeSessionId session_id, size_t index) {
    return (Bool)do_with_candidate_on_current_page(session_id, index, &Context::Highlight);
}

RIME_DEPRECATED Bool RimeSelectCandidate(RimeSessionId session_id, size_t index) {
    return (Bool)do_with_candidate(session_id, index, &Context::Select);
}

RIME_DEPRECATED Bool RimeSelectCandidateOnCurrentPage(RimeSessionId session_id, size_t index) {
    return (Bool)do_with_candidate_on_current_page(session_id, index, &Context::Select);
}

RIME_DEPRECATED Bool RimeDeleteCandidate(RimeSessionId session_id, size_t index) {
    return (Bool)do_with_candidate(session_id, index, &Context::DeleteCandidate);
}

RIME_DEPRECATED Bool RimeDeleteCandidateOnCurrentPage(RimeSessionId session_id, size_t index) {
    return (Bool)do_with_candidate_on_current_page(session_id, index, &Context::DeleteCandidate);
}
...
static RimeStringSlice RimeGetStateLabelAbbreviated(RimeSessionId session_id, const char* option_name, Bool state, Bool abbreviated) {
    an<Session> session(Service::instance().GetSession(session_id));
    if (!session)
        return {nullptr, 0};
...
static const char* RimeGetStateLabel(RimeSessionId session_id, const char* option_name, Bool state) {
