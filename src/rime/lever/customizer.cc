...
bool Customizer::UpdateConfigFile() {
    ...
    if (fs::exists(source_path_) && fs::exists(dest_path_) && fs::equivalent(source_path_, dest_path_)) {
    ...
    if (redistribute || (is_dirty && !missing_original_copy)) {
        try {
            fs::copy_file(source_path_, dest_path_, fs::copy_options::overwrite_existing);
        } catch (...) {
            LOG(ERROR) << "Error copying config file '" << source_path_ << "' to user directory.";
            return false;
        }
    }
