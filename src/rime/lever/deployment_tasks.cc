...
bool DetectModifications::Run(Deployer* deployer) {
    ...
    try {
        for (auto dir : data_dirs_) {
            ...
            last_modified = (std::max)(last_modified, filesystem::to_time_t(fs::last_write_time(p)));
            if (fs::is_directory(p)) {
                for (fs::directory_iterator iter(p), end; iter != end; ++iter) {
                    path entry(iter->path());
                    if (fs::is_regular_file(fs::canonical(entry)) && entry.extension().u8string() == ".yaml" && entry.filename().u8string() != "user.yaml") {
                        last_modified = (std::max)(last_modified, filesystem::to_time_t(fs::last_write_time(entry)));
                    }
                }
            }
        }
...
bool InstallationUpdate::Run(Deployer* deployer) {
    ...
    if (config.LoadFromFile(installation_info)) {
        if (config.GetString("installation_id", &installation_id)) {
            LOG(INFO) << "installation info exists. installation id: " << installation_id;
            // for now:
    ...
    if (!installation_id.empty() && last_distro_code_name == deployer->distribution_code_name && last_distro_version == deployer->distribution_version && last_rime_version == RIME_VERSION) {
        return true;
    }
    ...
    if (!deployer->distribution_code_name.empty()) {
        config.SetString("distribution_code_name", deployer->distribution_code_name);
...
bool WorkspaceUpdate::Run(Deployer* deployer) {
    ...
    the<ResourceResolver> resolver(Service::instance().CreateResourceResolver({"schema_source_file", "", ".schema.yaml"}));
    ...
    LOG(INFO) << "finished updating schemas: " << success << " success, " << failure << " failure.";
...
static bool TrashDeprecatedUserCopy(const path& shared_copy, const path& user_copy, const string& version_key, const path& trash) {
    if (!fs::exists(shared_copy) || !fs::exists(user_copy) || fs::equivalent(shared_copy, user_copy)) {
        return false;
    }
    ...
    Config user_config;
    bool is_customized_user_copy = user_config.LoadFromFile(user_copy) && user_config.GetString(version_key, &user_copy_version) && RemoveVersionSuffix(&user_copy_version, ".custom.");
...
bool SchemaUpdate::Run(Deployer* deployer) {
    ...
    if (!config->LoadFromFile(source_path_) || !config->GetString("schema/schema_id", &schema_id) || schema_id.empty()) {
        LOG(ERROR) << "invalid schema definition in '" << source_path_ << "'.";
        return false;
    }

    the<DeploymentTask> config_file_update(new ConfigFileUpdate(schema_id + ".schema.yaml", "schema/version"));
    ...
    the<Dictionary> dict(Dictionary::Require("dictionary")->Create({&schema, "translator"}));
    if (!dict) {
    ...
    the<ResourceResolver> resolver(Service::instance().CreateDeployedResourceResolver({"compiled_schema", "", ".schema.yaml"}));
...
static bool ConfigNeedsUpdate(Config* config) {
    ...
    the<ResourceResolver> resolver(Service::instance().CreateResourceResolver({"config_source_file", "", ".yaml"}));
    for (auto entry : *timestamps.AsMap()) {
        ...
        if (recorded_time != (int)filesystem::to_time_t(fs::last_write_time(source_file))) {
            LOG(INFO) << "source file " << (recorded_time ? "changed: " : "added: ") << source_file;
            return true;
        }
    }
    return false;
...
bool ConfigFileUpdate::Run(Deployer* deployer) {
    ...
    if (TrashDeprecatedUserCopy(source_config_path, dest_config_path, version_key_, trash)) {
        LOG(WARNING) << "deprecated user copy of '" << file_name_ << "' is moved to " << trash;
    }
...
bool PrebuildAllSchemas::Run(Deployer* deployer) {
    ...
    if (!fs::exists(shared_data_path) || !fs::is_directory(shared_data_path))
        return false;
    bool success = true;
    for (fs::directory_iterator iter(shared_data_path), end; iter != end; ++iter) {
        path entry(iter->path());
...
bool SymlinkingPrebuiltDictionaries::Run(Deployer* deployer) {
    const path shared_data_path(deployer->shared_data_dir);
    const path user_data_path(deployer->user_data_dir);
    if (!fs::exists(shared_data_path) || !fs::is_directory(shared_data_path) || !fs::exists(user_data_path) || !fs::is_directory(user_data_path) || fs::equivalent(shared_data_path, user_data_path))
        return false;
    bool success = false;
    ...
    for (fs::directory_iterator test(user_data_path), end; test != end; ++test) {
        ...
        if (fs::is_symlink(entry)) {
            try {
                ...
                bool linked_to_shared_data = !bad_link && target_path.has_parent_path() && fs::equivalent(shared_data_path, target_path.parent_path());
...
static bool IsCustomizedCopy(const path& file_path) {
    ...
    if (boost::ends_with(file_name, ".yaml") && !boost::ends_with(file_name, ".custom.yaml")) {
        Config config;
        string checksum;
        if (config.LoadFromFile(file_path) && config.GetString("customization", &checksum)) {
            return true;
        }
    }
    return false;
...
bool BackupConfigFiles::Run(Deployer* deployer) {
    ...
    LOG(INFO) << "backed up " << success << " config files to " << backup_dir << ", " << failure << " failed, " << latest << " up-to-date, " << skipped << " skipped.";
    return !failure;
}
...
bool CleanupTrash::Run(Deployer* deployer) {
    ...
    for (fs::directory_iterator iter(user_data_path), end; iter != end; ++iter) {
        ...
        if (file_name == "rime.log" || boost::ends_with(file_name, ".bin") || boost::ends_with(file_name, ".reverse.kct") || boost::ends_with(file_name, ".userdb.kct.old") || boost::ends_with(file_name, ".userdb.kct.snapshot")) {
    ...
    for (const auto& dir : dirs) {
        ...
        try {
            ...
            for (const auto& entry : fs::directory_iterator(dir)) {
                ...
                if (entry.is_regular_file() && !entry.is_symlink() && boost::starts_with(file_name, app_name) && boost::ends_with(file_name, ".log") && !boost::contains(file_name, today)) {
                    ...
                    if (boost::starts_with(target_file_name, app_name) && boost::ends_with(target_file_name, ".log")) {
