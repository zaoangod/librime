...
static RimeCustomSettings* rime_levers_custom_settings_init(const char* config_id, const char* generator_id) {
    return reinterpret_cast<RimeCustomSettings*>(new CustomSettings(&Service::instance().deployer(), config_id, generator_id));
}
...
static Bool rime_levers_customize_bool(RimeCustomSettings* settings, const char* key, Bool value) {
...
static Bool rime_levers_customize_int(RimeCustomSettings* settings, const char* key, int value) {
    an<ConfigItem> item = New<ConfigValue>(value);
    auto custom_settings = reinterpret_cast<CustomSettings*>(settings);
    return custom_settings->Customize(key, item);
}

static Bool rime_levers_customize_double(RimeCustomSettings* settings, const char* key, double value) {
    an<ConfigItem> item = New<ConfigValue>(value);
    auto custom_settings = reinterpret_cast<CustomSettings*>(settings);
    return custom_settings->Customize(key, item);
}

static Bool rime_levers_customize_string(RimeCustomSettings* settings, const char* key, const char* value) {
    an<ConfigItem> item = New<ConfigValue>(value);
    auto custom_settings = reinterpret_cast<CustomSettings*>(settings);
    return custom_settings->Customize(key, item);
}

static Bool rime_levers_customize_item(RimeCustomSettings* settings, const char* key, RimeConfig* value) {
    an<ConfigItem> item;
    if (value) {
...
static RimeSwitcherSettings* rime_levers_switcher_settings_init() {
    return reinterpret_cast<RimeSwitcherSettings*>(new SwitcherSettings(&Service::instance().deployer()));
}

static Bool rime_levers_get_available_schema_list(RimeSwitcherSettings* settings, RimeSchemaList* list) {
...
static Bool rime_levers_get_selected_schema_list(RimeSwitcherSettings* settings, RimeSchemaList* list) {
    auto ss = reinterpret_cast<SwitcherSettings*>(settings);
    list->size = 0;
    list->list = NULL;
...
static Bool rime_levers_select_schemas(RimeSwitcherSettings* settings, const char* schema_id_list[], int count) {
    auto ss = reinterpret_cast<SwitcherSettings*>(settings);
