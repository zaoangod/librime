...
class RIME_DLL InstallationUpdate : public DeploymentTask {
   public:
...
class RIME_DLL WorkspaceUpdate : public DeploymentTask {
   ...
   protected:
    string GetSchemaPath(Deployer* deployer, const string& schema_id, bool prefer_shared_copy);
};
...
class ConfigFileUpdate : public DeploymentTask {
   public:
    ConfigFileUpdate(const string& file_name, const string& version_key) : file_name_(file_name), version_key_(version_key) {}
