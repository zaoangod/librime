...
UserDictManager::UserDictManager(Deployer* deployer) : deployer_(deployer), user_db_component_(UserDb::Require("userdb")) {
    if (deployer) {
        path_ = deployer->user_data_dir;
    }
}

void UserDictManager::GetUserDictList(UserDictList* user_dict_list, UserDb::Component* component) {
    if (!user_dict_list)
        return;
    if (!component) {
...
bool UserDictManager::Restore(const path& snapshot_file) {
    ...
    BOOST_SCOPE_EXIT_END
    LOG(INFO) << "merging '" << snapshot_file << "' from " << UserDbHelper(temp).GetUserId() << " into userdb '" << db_name << "'...";
...
bool UserDictManager::UpgradeUserDict(const string& dict_name) {
    ...
    return legacy_db->Backup(snapshot_path) && legacy_db->Close() && legacy_db->Remove() && Restore(snapshot_path);
}
...
bool UserDictManager::SynchronizeAll() {
    ...
    if (failure) {
        LOG(ERROR) << "failed synchronizing " << failure << "/" << user_dicts.size() << " user dicts.";
    }
    return !failure;
}
