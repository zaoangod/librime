...
void Segmentation::Reset(const string& new_input) {
    ...
    size_t diff_pos = 0;
    while (diff_pos < input_.length() && diff_pos < new_input.length() && input_[diff_pos] == new_input[diff_pos])
        ++diff_pos;
...
bool Segmentation::AddSegment(Segment segment) {
    ...
    if (last.end > segment.end) {
        ...
        set<string> result;
        set_union(last.tags.begin(), last.tags.end(), segment.tags.begin(), segment.tags.end(), std::inserter(result, result.begin()));
