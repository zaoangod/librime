...
Session::Session() {
    ...
    engine_->message_sink().connect([session_id](auto type, auto value) { Service::instance().Notify(session_id, type, value); });
}
...
Service::Service() {
    deployer_.message_sink().connect([this](auto type, auto value) { Notify(0, type, value); });
}
...
void Service::Notify(SessionId session_id, const string& message_type, const string& message_value) {
    if (notification_handler_) {
        std::lock_guard<std::mutex> lock(mutex_);
        notification_handler_(session_id, message_type.c_str(), message_value.c_str());
    }
}
...
ResourceResolver* Service::CreateUserSpecificResourceResolver(const ResourceType& type) {
    the<ResourceResolver> resolver(new ResourceResolver(type));
    resolver->set_root_path(deployer().user_data_dir);
    return resolver.release();
}

ResourceResolver* Service::CreateDeployedResourceResolver(const ResourceType& type) {
    the<FallbackResourceResolver> resolver(new FallbackResourceResolver(type));
...
ResourceResolver* Service::CreateStagingResourceResolver(const ResourceType& type) {
    the<ResourceResolver> resolver(new ResourceResolver(type));
    resolver->set_root_path(deployer().staging_dir);
    return resolver.release();
}
