...
bool Context::Highlight(size_t index) {
    ...
    size_t new_index = candidate_count > 0 ? (std::min)(candidate_count - 1, index) : 0;
...
bool Context::ConfirmCurrentSelection() {
    ...
    if (auto cand = seg.GetSelectedCandidate()) {
        DLOG(INFO) << "Confirmed: '" << cand->text() << "', selected_index = " << seg.selected_index;
    } else {
...
bool Context::ReopenPreviousSegment() {
    if (composition_.Trim()) {
        if (!composition_.empty() && composition_.back().status >= Segment::kSelected) {
...
bool Context::ClearNonConfirmedComposition() {
    bool reverted = false;
    while (!composition_.empty() && composition_.back().status < Segment::kSelected) {
        composition_.pop_back();
        reverted = true;
    }
    if (reverted) {
...
void Context::ClearTransientOptions() {
    ...
    while (prop != properties_.end() && !prop->first.empty() && prop->first[0] == '_') {
