...
struct Segment {
    ...
    Segment(int start_pos, int end_pos) : start(start_pos), end(end_pos), length(end_pos - start_pos) {}
    ...
    bool HasAnyTagIn(const vector<string>& tags) const {
        return std::any_of(tags.begin(), tags.end(), [this](const string& tag) { return HasTag(tag); });
    }
