...
SpellingAccessor::SpellingAccessor(prism::SpellingMap* spelling_map, SyllableId spelling_id) : spelling_id_(spelling_id), iter_(NULL), end_(NULL) {
    if (spelling_map && spelling_id < static_cast<SyllableId>(spelling_map->size)) {
...
Prism::Prism(const path& file_path) : MappedFile(file_path), trie_(new Darts::DoubleArray) {}
...
bool Prism::Build(const Syllabary& syllabary, const Script* script, uint32_t dict_file_checksum, uint32_t schema_file_checksum) {
    ...
    size_t estimated_map_size = num_spellings * 12 + map_size * (4 + sizeof(prism::SpellingDescriptor) + kDescriptorExtraSize);
    ...
    if (script) {
        ...
        for (; i != script->end(); ++i, ++item) {
            ...
            for (; j != i->second.end(); ++j, ++desc) {
                ...
                if (!j->properties.tips.empty() && !CopyString(j->properties.tips, &desc->tips)) {
...
void Prism::ExpandSearch(const string& key, vector<Match>* result, size_t limit) {
    if (!result)
        return;
    result->clear();
    size_t count = 0;
    ...
    while (!q.empty()) {
        node_t node = q.front();
        q.pop();
        const char* c = (format_ > 1.0 - DBL_EPSILON) ? metadata_->alphabet : kDefaultAlphabet;
        for (; *c; ++c) {
            string k = node.key + *c;
