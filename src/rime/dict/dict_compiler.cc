...
DictCompiler::DictCompiler(Dictionary* dictionary) : dict_name_(dictionary->name()), packs_(dictionary->packs()), prism_(dictionary->prism()), tables_(dictionary->tables()), source_resolver_(Service::instance().CreateResourceResolver({"source_file", "", ""})), target_resolver_(Service::instance().CreateStagingResourceResolver({"target_file", "", ""})) {}

DictCompiler::~DictCompiler() {}

static bool load_dict_settings_from_file(DictSettings* settings, const path& dict_file) {
...
static bool get_dict_files_from_settings(vector<path>* dict_files, DictSettings& settings, ResourceResolver* source_resolver) {
...
static uint32_t compute_dict_file_checksum(uint32_t initial_checksum, const vector<path>& dict_files, DictSettings& settings) {
    if (dict_files.empty()) {
...
bool DictCompiler::Compile(const path& schema_file) {
    ...
    if (!get_dict_files_from_settings(&dict_files, settings, source_resolver_.get())) {
        return false;
    }
    uint32_t dict_file_checksum = compute_dict_file_checksum(0, dict_files, settings);
    ...
    if (primary_table->Exists() && primary_table->Load()) {
        ...
        LOG(ERROR) << "neither " << dict_name_ << ".dict.yaml nor " << dict_name_ << ".table.bin exists.";
        return false;
    }
    if (prism_->Exists() && prism_->Load()) {
        rebuild_prism = prism_->dict_file_checksum() != dict_file_checksum || prism_->schema_file_checksum() != schema_file_checksum;
        prism_->Close();
    } else {
        rebuild_prism = true;
    }
    ...
    {
        the<ResourceResolver> resolver(Service::instance().CreateDeployedResourceResolver({"find_reverse_db", "", ".reverse.bin"}));
        ReverseDb reverse_db(resolver->ResolvePath(dict_name_));
        if (!reverse_db.Exists() || !reverse_db.Load() || reverse_db.dict_file_checksum() != dict_file_checksum) {
            rebuild_table = true;
        }
    }
    ...
    for (int table_index = 1; table_index < tables_.size(); ++table_index) {
        ...
        if (!std::filesystem::exists(dict_file)) {
            if (pack_table->Exists())
                LOG(INFO) << "pack source file '" << dict_file << "' does not exist, using prebuilt table '" << pack_table->file_path() << "'";
            else
                LOG(ERROR) << "neither pack source file '" << dict_file << "' nor a prebuilt table exists";
            continue;
        }
        ...
        uint32_t pack_file_checksum = compute_dict_file_checksum(dict_file_checksum, dict_files, settings);
        ...
        if (rebuild_pack) {
            ...
            if (!BuildTable(table_index, collector, &settings, dict_files, pack_file_checksum)) {
                LOG(ERROR) << "failed to build pack: " << pack_name;
            }
        } else {
            LOG(INFO) << "pack '" << pack_name << "' reuses up-to-date table '" << pack_table->file_path() << "'";
        }
        syllabary = std::move(collector.syllabary);
        pack_table->Close();
    }
...
static path relocate_target(const path& source_path, ResourceResolver* target_resolver) {
    auto resource_id = source_path.filename().u8string();
    return target_resolver->ResolvePath(resource_id);
}

bool DictCompiler::BuildTable(int table_index, EntryCollector& collector, DictSettings* settings, const vector<path>& dict_files, uint32_t dict_file_checksum) {
    auto& table = tables_[table_index];
    auto target_path = relocate_target(table->file_path(), target_resolver_.get());
    ...
    // build .table.bin
    {
        ...
        table->Remove();
        if (!table->Build(collector.syllabary, vocabulary, collector.num_entries, dict_file_checksum) || !table->Save()) {
            return false;
        }
    }
    // build reverse db for the primary table
    if (table_index == 0 && !BuildReverseDb(settings, collector, vocabulary, dict_file_checksum)) {
        return false;
    }
    return true;
}

bool DictCompiler::BuildReverseDb(DictSettings* settings, const EntryCollector& collector, const Vocabulary& vocabulary, uint32_t dict_file_checksum) {
    // build .reverse.bin
    auto target_path = target_resolver_->ResolvePath(dict_name_ + ".reverse.bin");
    ReverseDb reverse_db(target_path);
    if (!reverse_db.Build(settings, collector.syllabary, vocabulary, collector.stems, dict_file_checksum) || !reverse_db.Save()) {
...
bool DictCompiler::BuildPrism(const path& schema_file, uint32_t dict_file_checksum, uint32_t schema_file_checksum) {
    LOG(INFO) << "building prism...";
    auto target_path = relocate_target(prism_->file_path(), target_resolver_.get());
    prism_ = New<Prism>(target_path);

    // get syllabary from primary table, which may not be rebuilt
    Syllabary syllabary;
    const auto& primary_table = tables_[0];
    if (!primary_table->Load() || !primary_table->GetSyllabary(&syllabary) || syllabary.empty())
        return false;
    ...
    {
        prism_->Remove();
        if (!prism_->Build(syllabary, script.empty() ? nullptr : &script, dict_file_checksum, schema_file_checksum) || !prism_->Save()) {
            return false;
        }
    }
