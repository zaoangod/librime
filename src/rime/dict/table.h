...
class TableAccessor {
   public:
    ...
    TableAccessor(const Code& index_code, const List<table::Entry>* entries, double credibility = 0.0);
    TableAccessor(const Code& index_code, const Array<table::Entry>* entries, double credibility = 0.0);
    TableAccessor(const Code& index_code, const table::TailIndex* code_map, double credibility = 0.0);
...
class Table : public MappedFile {
   ...
   private:
    ...
    bool BuildPhraseIndex(Code code, const Vocabulary& vocabulary, map<string, int>* index_data);
