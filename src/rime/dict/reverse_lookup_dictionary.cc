...
bool ReverseDb::Load() {
    ...
    if (strncmp(metadata_->format, kReverseFormatPrefix, kReverseFormatPrefixLen)) {
        LOG(ERROR) << "invalid metadata.";
        Close();
        return false;
    }
    double format = std::atof(&metadata_->format[kReverseFormatPrefixLen]);
    if (format - kReverseFormatCompatible < 0.0 - DBL_EPSILON || format - kReverseFormatCompatible > 1.0 + DBL_EPSILON) {
        LOG(ERROR) << "incompatible reversedb format.";
        Close();
        return false;
    }

    key_trie_.reset(new StringTable(metadata_->key_trie.get(), metadata_->key_trie_size));
    value_trie_.reset(new StringTable(metadata_->value_trie.get(), metadata_->value_trie_size));
...
bool ReverseDb::Build(DictSettings* settings, const Syllabary& syllabary, const Vocabulary& vocabulary, const ReverseLookupTable& stems, uint32_t dict_file_checksum) {
    ...
    size_t estimated_data_size = kReservedSize + dict_settings.length() + entry_count * sizeof(StringId) + key_trie_image_size + value_trie_image_size;
    ...
    std::strncpy(metadata_->format, kReverseFormat, reverse::Metadata::kFormatMaxLength);
    return true;
}
...
bool ReverseLookupDictionary::ReverseLookup(const string& text, string* result) {
...
static const ResourceType kReverseDbResourceType = {"reverse_db", "", ".reverse.bin"};

ReverseLookupDictionaryComponent::ReverseLookupDictionaryComponent() : DbPool(the<ResourceResolver>(Service::instance().CreateDeployedResourceResolver(kReverseDbResourceType))) {}

ReverseLookupDictionary* ReverseLookupDictionaryComponent::Create(const string& dict_name) {
    auto db = GetDb(dict_name);
    return new ReverseLookupDictionary(db);
};

ReverseLookupDictionary* ReverseLookupDictionaryComponent::Create(const Ticket& ticket) {
    if (!ticket.schema)
        return NULL;
