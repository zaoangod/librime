...
inline void sort(C& container) {
    std::stable_sort(std::begin(container), std::end(container), dereference_less<typename C::value_type>);
}
...
void DictEntryFilterBinder::AddFilter(DictEntryFilter filter) {
    if (!filter_) {
        ...
        filter_ = [previous_filter, filter](an<DictEntry> e) { return previous_filter(e) && filter(e); };
    }
}
