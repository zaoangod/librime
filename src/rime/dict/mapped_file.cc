...
class MappedFileImpl {
   public:
    ...
    MappedFileImpl(const path& file_path, OpenMode mode) {
        boost::interprocess::mode_t file_mapping_mode = (mode == kOpenReadOnly) ? boost::interprocess::read_only : boost::interprocess::read_write;
        file_.reset(new boost::interprocess::file_mapping(file_path.c_str(), file_mapping_mode));
...
bool MappedFile::Create(size_t capacity) {
    if (Exists()) {
        ...
        std::filebuf fbuf;
        fbuf.open(file_path_.c_str(), std::ios_base::in | std::ios_base::out | std::ios_base::trunc | std::ios_base::binary);
        if (capacity > 0) {
