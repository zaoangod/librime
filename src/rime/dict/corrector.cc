...
void EditDistanceCorrector::ToleranceSearch(const Prism& prism, const string& key, Corrections* results, size_t threshold) {
    if (key.empty())
        return;
...
Distance EditDistanceCorrector::LevenshteinDistance(const std::string& s1, const std::string& s2) {
    ...
    for (auto x = column_start; x <= s2len; x++) {
        ...
        for (auto y = column_start; y <= s1len; y++) {
            ...
            auto possibilities = {column[y] + 1, column[y - 1] + 1, last_diagonal + SubstCost(s1[y - 1], s2[x - 1])};
...
Distance EditDistanceCorrector::RestrictedDistance(const std::string& s1, const std::string& s2, Distance threshold) {
    ...
    for (size_t i = 1; i <= len1; ++i) {
        ...
        for (size_t j = 1; j <= len2; ++j) {
            d[index(i, j)] = (std::min)({d[index(i - 1, j)] + 2, d[index(i, j - 1)] + 2, d[index(i - 1, j - 1)] + SubstCost(s1[i - 1], s2[j - 1])});
...
bool EditDistanceCorrector::Build(const Syllabary& syllabary, const Script* script, uint32_t dict_file_checksum, uint32_t schema_file_checksum) {
...
EditDistanceCorrector::EditDistanceCorrector(const path& file_path) : Prism(file_path) {}

void NearSearchCorrector::ToleranceSearch(const Prism& prism, const string& key, Corrections* results, size_t threshold) {
    if (key.empty())
        return;
...
void CorrectorComponent::Unified::ToleranceSearch(const Prism& prism, const string& key, Corrections* results, size_t tolerance) {
    for (auto& c : contents) {
        c->ToleranceSearch(prism, key, results, tolerance);
    }
}
CorrectorComponent::CorrectorComponent() : resolver_(Service::instance().CreateDeployedResourceResolver({"corrector", "", ".correction.bin"})) {}
