...
LevelDbAccessor::LevelDbAccessor(LevelDbCursor* cursor, const string& prefix) : DbAccessor(prefix), cursor_(cursor), is_metadata_query_(prefix == kMetaCharacter) {
    Reset();
}
...
LevelDb::LevelDb(const path& file_path, const string& db_name, const string& db_type) : Db(file_path, db_name), db_type_(db_type) {}
...
bool LevelDb::Backup(const path& snapshot_file) {
    ...
    if (!success) {
        LOG(ERROR) << "failed to create snapshot file '" << snapshot_file << "' for db '" << name() << "'.";
    }
    return success;
}
...
bool LevelDb::Restore(const path& snapshot_file) {
    ...
    if (!success) {
        LOG(ERROR) << "failed to restore db '" << name() << "' from '" << snapshot_file << "'.";
    }
    return success;
}
...
template <>
RIME_DLL UserDbWrapper<LevelDb>::UserDbWrapper(const path& file_path, const string& db_name) : LevelDb(file_path, db_name, "userdb") {}
