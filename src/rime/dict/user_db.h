...
class UserDb {
   public:
    ...
    static Component* Require(const string& name) { return dynamic_cast<Component*>(Db::Require(name)); }
...
class UserDbWrapper : public BaseDb {
   public:
    ...
    virtual bool CreateMetadata() { return BaseDb::CreateMetadata() && UserDbHelper(this).UpdateUserInfo(); }
    virtual bool Backup(const path& snapshot_file) { return UserDbHelper::IsUniformFormat(snapshot_file) ? UserDbHelper(this).UniformBackup(snapshot_file) : BaseDb::Backup(snapshot_file); }
    virtual bool Restore(const path& snapshot_file) { return UserDbHelper::IsUniformFormat(snapshot_file) ? UserDbHelper(this).UniformRestore(snapshot_file) : BaseDb::Restore(snapshot_file); }
};
...
class UserDbComponent : public UserDb::Component, protected DbComponentBase {
   public:
    ...
    Db* Create(const string& name) override { return new UserDbImpl(DbFilePath(name, extension()), name); }
