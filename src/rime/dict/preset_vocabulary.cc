...
VocabularyDb::VocabularyDb(const path& file_path, const string& db_name) : TextDb(file_path, db_name, kVocabularyResourceType.name, VocabularyDb::format) {}

static bool rime_vocabulary_entry_parser(const Tsv& row, string* key, string* value) {
...
static bool rime_vocabulary_entry_formatter(const string& key, const string& value, Tsv* tsv) {
    // Tsv& row(*tsv);
    // row.push_back(key);
    // row.push_back(value);
    return true;
...
path PresetVocabulary::DictFilePath(const string& vocabulary) {
    the<ResourceResolver> resource_resolver(Service::instance().CreateResourceResolver(kVocabularyResourceType));
...
bool PresetVocabulary::IsQualifiedPhrase(const string& phrase, const string& weight_str) {
    if (max_phrase_length_ > 0) {
        size_t length = utf8::unchecked::distance(phrase.c_str(), phrase.c_str() + phrase.length());
