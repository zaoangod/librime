...
DbComponentBase::DbComponentBase() : db_resource_resolver_(Service::instance().CreateResourceResolver(kDbResourceType)) {}

DbComponentBase::~DbComponentBase() {}

path DbComponentBase::DbFilePath(const string& name, const string& extension) const {
    return db_resource_resolver_->ResolvePath(name + extension);
}

// Db members

Db::Db(const path& file_path, const string& name) : name_(name), file_path_(file_path) {}
...
bool Db::CreateMetadata() {
    ...
    return MetaUpdate("/db_name", name_) && MetaUpdate("/rime_version", RIME_VERSION);
}
