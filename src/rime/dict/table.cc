...
TableAccessor::TableAccessor(const Code& index_code, const List<table::Entry>* list, double credibility) : index_code_(index_code), entries_(list->at.get()), size_(list->size), credibility_(credibility) {}

TableAccessor::TableAccessor(const Code& index_code, const Array<table::Entry>* array, double credibility) : index_code_(index_code), entries_(array->at), size_(array->size), credibility_(credibility) {}

TableAccessor::TableAccessor(const Code& index_code, const table::TailIndex* code_map, double credibility) : index_code_(index_code), long_entries_(code_map->at), size_(code_map->size), credibility_(credibility) {}
...
inline static bool node_less(const table::TrunkIndexNode& a, const table::TrunkIndexNode& b) {
    return a.key < b.key;
}

static table::TrunkIndexNode* find_node(table::TrunkIndexNode* first, table::TrunkIndexNode* last, const SyllableId& key) {
...
bool TableQuery::Walk(SyllableId syllable_id) {
    if (level_ == 0) {
        if (!lv1_index_ || syllable_id < 0 || syllable_id >= static_cast<SyllableId>(lv1_index_->size))
            return false;
...
TableAccessor TableQuery::Access(SyllableId syllable_id, double credibility) const {
    credibility += credibility_.back();
    if (level_ == 0) {
        if (!lv1_index_ || syllable_id < 0 || syllable_id >= static_cast<SyllableId>(lv1_index_->size))
            return TableAccessor();
        auto node = &lv1_index_->at[syllable_id];
        return TableAccessor(add_syllable(index_code_, syllable_id), &node->entries, credibility);
...
bool Table::AddString(const string& src, table::StringType* dest, double weight) {
...
bool Table::OnLoad() {
    string_table_.reset(new StringTable(metadata_->string_table.get(), metadata_->string_table_size));
    return true;
}
...
bool Table::Load() {
    ...
    if (format_version < kTableFormatLowestCompatible - DBL_EPSILON) {
        LOG(ERROR) << "table format version " << format_version << " is no longer supported. please upgrade to version " << kTableFormatLatest;
        return false;
    }
...
bool Table::Build(const Syllabary& syllabary, const Vocabulary& vocabulary, size_t num_entries, uint32_t dict_file_checksum) {
    ...
    std::strncpy(metadata_->format, kTableFormatLatest, table::Metadata::kFormatMaxLength);
    return true;
}

table::Index* Table::BuildIndex(const Vocabulary& vocabulary, size_t num_syllables) {
    return reinterpret_cast<table::Index*>(BuildHeadIndex(vocabulary, num_syllables));
}
...
table::TailIndex* Table::BuildTailIndex(const Code& prefix, const Vocabulary& vocabulary) {
    ...
    for (const auto& src : page.entries) {
        ...
        std::copy(src->code.begin() + Code::kIndexCodeMaxLength, src->code.end(), dest.extra_code.begin());
...
bool Table::BuildEntryList(const ShortDictEntryList& src, List<table::Entry>* dest) {
    if (!dest)
        return false;
    dest->size = src.size();
...
string Table::GetSyllableById(SyllableId syllable_id) {
    if (!syllabary_ || syllable_id < 0 || syllable_id >= static_cast<SyllableId>(syllabary_->size))
        return string();
...
bool Table::Query(const SyllableGraph& syll_graph, size_t start_pos, TableQueryResult* result) {
    ...
    while (!q.empty()) {
        ...
        for (const auto& spellings : index->second) {
            ...
            for (auto props : spellings.second) {
                ...
                if (end_pos < syll_graph.interpreted_length && query.Advance(syll_id, props->credibility)) {
