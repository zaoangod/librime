...
struct Chunk {
    ...
    Chunk() = default;
    Chunk(Table* t, const Code& c, const table::Entry* e, size_t m, double cr = 0.0) : table(t), code(c), entries(e), size(1), cursor(0), matching_code_size(m), credibility(cr) {}
    Chunk(Table* t, const TableAccessor& a, double cr = 0.0) : Chunk(t, a, string(), cr) {}
    Chunk(Table* t, const TableAccessor& a, const string& r, double cr = 0.0) : table(t), code(a.index_code()), entries(a.entry()), size(a.remaining()), cursor(0), remaining_code(r), matching_code_size(a.index_code().size()), credibility(cr) {}
...
bool compare_chunk_by_head_element(const Chunk& a, const Chunk& b) {
    ...
    return a.credibility + a.entries[a.cursor].weight > b.credibility + b.entries[b.cursor].weight;  // by weight desc
}
...
CodeMatch match_extra_code(const table::Code* extra_code, size_t depth, const SyllableGraph& syll_graph, size_t current_pos, bool predict_word) {
    ...
    for (const SpellingProperties* props : spellings->second) {
        CodeMatch match = match_extra_code(extra_code, depth + 1, syll_graph, props->end_pos, predict_word);
        if (!match.success)
            continue;
...
DictEntryIterator::DictEntryIterator() : query_result_(New<dictionary::QueryResult>()) {}
...
void DictEntryIterator::Sort() {
    ...
    std::partial_sort(chunks.begin() + chunk_index_, chunks.begin() + chunk_index_ + 1, chunks.end(), dictionary::compare_chunk_by_head_element);
}
...
an<DictEntry> DictEntryIterator::Peek() {
    if (!entry_ && !exhausted()) {
        ...
        DLOG(INFO) << "creating temporary dict entry '" << chunk.table->GetEntryText(e) << "'.";
...
Dictionary::Dictionary(string name, vector<string> packs, vector<of<Table>> tables, an<Prism> prism) : name_(name), packs_(std::move(packs)), tables_(std::move(tables)), prism_(std::move(prism)) {}

Dictionary::~Dictionary() {
    // should not close shared table and prism objects
}

static void lookup_table(Table* table, DictEntryCollector* collector, const SyllableGraph& syllable_graph, size_t start_pos, bool predict_word, double initial_credibility) {
    ...
    for (auto& v : result) {
        ...
        for (TableAccessor& a : v.second) {
            ...
            if (a.extra_code()) {
                do {
                    dictionary::CodeMatch match = dictionary::match_extra_code(a.extra_code(), 0, syllable_graph, end_pos, predict_word);
                    if (!match.success)
                        continue;
                    size_t matching_code_size = a.index_code().size() + match.depth;
                    (*collector)[match.end_pos].AddChunk({table, a.code(), a.entry(), matching_code_size, cr});
                } while (a.Next());
            } else {
...
an<DictEntryCollector> Dictionary::Lookup(const SyllableGraph& syllable_graph, size_t start_pos, const hash_set<string>* blacklist, bool predict_word, double initial_credibility) {
    if (!loaded())
        return nullptr;
    ...
    for (const auto& table : tables_) {
        if (!table->IsOpen())
            continue;
        lookup_table(table.get(), collector.get(), syllable_graph, start_pos, predict_word, initial_credibility);
    }
    if (collector->empty())
        return nullptr;
    ...
    for (auto& v : *collector) {
        ...
        if (blacklist && !blacklist->empty()) {
            v.second.AddFilter([blacklist](an<DictEntry> entry) { return entry && !blacklist->count(entry->text); });
        }
    }
    return collector;
}

size_t Dictionary::LookupWords(DictEntryIterator* result, const string& str_code, bool predictive, size_t expand_search_limit, const hash_set<string>* blacklist) {
    ...
    if (blacklist && !blacklist->empty()) {
        result->AddFilter([blacklist](an<DictEntry> entry) { return entry && !blacklist->count(entry->text); });
    }
    return keys.size();
}
...
bool Dictionary::Exists() const {
    return std::filesystem::exists(prism_->file_path()) && !tables_.empty() && std::filesystem::exists(tables_[0]->file_path());
}
...
bool Dictionary::Load() {
    ...
    if (tables_.empty()) {
        LOG(ERROR) << "Cannot load dictionary '" << name_ << "'; it contains no tables.";
        return false;
    }
...
DictionaryComponent::DictionaryComponent() : prism_resource_resolver_(Service::instance().CreateDeployedResourceResolver(kPrismResourceType)), table_resource_resolver_(Service::instance().CreateDeployedResourceResolver(kTableResourceType)) {}
...
Dictionary* DictionaryComponent::Create(const Ticket& ticket) {
    ...
    if (!config->GetString(ticket.name_space + "/dictionary", &dict_name)) {
        LOG(ERROR) << ticket.name_space << "/dictionary not specified in schema '" << ticket.schema->schema_id() << "'.";
        return nullptr;
    }
    if (dict_name.empty()) {
...
Dictionary* DictionaryComponent::Create(string dict_name, string prism_name, vector<string> packs) {
    ...
    return new Dictionary(std::move(dict_name), std::move(packs), std::move(tables), std::move(prism));
}
