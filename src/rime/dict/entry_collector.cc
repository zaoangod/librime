...
EntryCollector::EntryCollector(Syllabary&& fixed_syllabary) : syllabary(std::move(fixed_syllabary)), build_syllabary(false) {}
...
void EntryCollector::Collect(const path& dict_file) {
    ...
    while (getline(fin, line)) {
        ...
        if (num_columns <= text_column || row[text_column].empty()) {
            LOG(WARNING) << "Missing entry text at #" << num_entries << ", line: " << line_number << " of file: " << current_dict_file << ".";
            continue;
        }
        ...
        if (weight_column != -1 && num_columns > weight_column && !row[weight_column].empty())
            weight_str = row[weight_column];
        if (stem_column != -1 && num_columns > stem_column && !row[stem_column].empty())
...
void EntryCollector::CreateEntry(const string& word, const string& code_str, const string& weight_str) {
    ...
    if (scaled) {
        ...
        try {
            ...
            LOG(WARNING) << "invalid entry definition at #" << num_entries << ", line: " << line_number << " of file: " << current_dict_file << ".";
            percentage = 100.0;
        }
    ...
    for (const string& s : e->raw_code) {
        if (syllabary.find(s) == syllabary.end()) {
            if (build_syllabary) {
                ...
                LOG(ERROR) << "dropping entry '" << e->text << "' with invalid syllable: " << s;
                return;
            }
        }
    }
    ...
    if (is_word) {
        ...
        if (std::find_if(weights.begin(), weights.end(), [&](const auto& p) { return p.first == code_str; }) != weights.end()) {
...
bool EntryCollector::TranslateWord(const string& word, vector<string>* result) {
    ...
    if (w != words.end()) {
        std::sort(w->second.begin(), w->second.end(), [](const auto& a, const auto& b) { return a.first < b.first; });
...
void EntryCollector::Dump(const path& file_path) const {
    ...
    for (const auto& e : entries) {
        out << e->text << '\t' << e->raw_code.ToString() << '\t' << e->weight << std::endl;
    }
    out.close();
}
