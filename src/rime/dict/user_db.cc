...
static bool userdb_entry_formatter(const string& key, const string& value, Tsv* tsv) {
    Tsv& row(*tsv);
...
template <>
RIME_DLL UserDbWrapper<TextDb>::UserDbWrapper(const path& file_path, const string& db_name) : TextDb(file_path, db_name, "userdb", plain_userdb_format) {}
...
bool UserDbHelper::IsUniformFormat(const path& file_path) {
    return boost::ends_with(file_path.filename().u8string(), plain_userdb_extension);
}
...
bool UserDbHelper::UniformRestore(const path& snapshot_file) {
    LOG(INFO) << "restoring userdb '" << db_->name() << "' from " << snapshot_file;
...
void UserDbMerger::CloseMerge() {
    ...
    LOG(INFO) << "total " << merged_entries_ << " entries merged, tick = " << max_tick_;
    merged_entries_ = 0;
}
