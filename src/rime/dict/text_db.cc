...
TextDbAccessor::TextDbAccessor(const TextDbData& data, const string& prefix) : DbAccessor(prefix), data_(data) {
    Reset();
}
...
TextDb::TextDb(const path& file_path, const string& db_name, const string& db_type, TextFormat format) : Db(file_path, db_name), db_type_(db_type), format_(format) {}
