...
struct DfsState {
    ...
    bool IsExactMatch(const string& prefix) { return boost::starts_with(key, prefix + '\t'); }
    bool IsPrefixMatch(const string& prefix) { return boost::starts_with(key, prefix); }
...
void DfsState::RecruitEntry(size_t pos, hash_map<string, SyllableId>* syllabary) {
    string full_code;
    auto e = UserDictionary::CreateDictEntry(key, value, present_tick, credibility.back(), syllabary ? &full_code : nullptr);
    if (e) {
        if (syllabary) {
            vector<string> syllables = strings::split(full_code, " ", strings::SplitBehavior::SkipToken);
            Code numeric_code;
            for (auto s = syllables.begin(); s != syllables.end(); ++s) {
                auto found = syllabary->find(*s);
                if (found == syllabary->end()) {
                    LOG(ERROR) << "failed to recruit dict entry '" << e->text << "', unrecognized syllable: " << *s;
                    return;
                }
...
UserDictionary::UserDictionary(const string& name, an<Db> db) : name_(name), db_(db) {}
...
void UserDictionary::DfsLookup(const SyllableGraph& syll_graph, size_t current_pos, const string& current_prefix, DfsState* state) {
    ...
    for (const auto& spelling : index->second) {
        DLOG(INFO) << "prefix: '" << current_prefix << "'"
                   << ", syll_id: " << spelling.first << ", num_spellings: " << spelling.second.size();
        ...
        for (size_t i = 0; i < spelling.second.size(); ++i) {
            ...
            if (next_index == syll_graph.indices.end()) {
                ...
                if (state->predict_word_from_depth != 0 && state->depth() >= state->predict_word_from_depth) {
                ...
                if ((!state->depth_limit || state->depth() < state->depth_limit) && state->IsPrefixMatch(prefix)) {  // 'b |e ' vs. 'b e f \tBefore'
...
an<UserDictEntryCollector> UserDictionary::Lookup(const SyllableGraph& syll_graph, size_t start_pos, size_t depth_limit, size_t predict_word_from_depth, double initial_credibility) {
    if (!table_ || !prism_ || !loaded() || start_pos >= syll_graph.interpreted_length)
        return nullptr;
    DfsState state;
    ...
    for (auto& v : state.query_result) {
        ...
        if (state.predict_word_from_depth) {
            if (!entries.empty() && entries.front()->IsPredictiveMatch()) {
                DLOG(INFO) << "front entry is predictive match: " << entries.front()->text;
                auto found = std::find_if(entries.begin(), entries.end(), [](const auto& e) { return e->IsExactMatch(); });
                if (found != entries.end()) {
                    DLOG(INFO) << "rotating exact match entry to front: " << (*found)->text;
...
size_t UserDictionary::LookupWords(UserDictEntryIterator* result, const string& input, bool predictive, size_t limit, string* resume_key) {
    ...
    if (resume_key && !resume_key->empty()) {
        if (!accessor->Jump(*resume_key) || !accessor->GetNextRecord(&key, &value)) {
            *resume_key = kEnd;
            return 0;
        }
...
bool UserDictionary::UpdateEntry(const DictEntry& entry, int commits, const string& new_entry_prefix) {
...
bool UserDictionary::TranslateCodeToString(const Code& code, string* result) {
    ...
    for (const SyllableId& syllable_id : code) {
        string spelling = rev_syllabary_.count(syllable_id) ? rev_syllabary_[syllable_id] : (rev_syllabary_[syllable_id] = table_->GetSyllableById(syllable_id));
        if (spelling.empty()) {
...
an<DictEntry> UserDictionary::CreateDictEntry(const string& key, const string& value, TickCount present_tick, double credibility, string* full_code) {
    an<DictEntry> e;
    ...
    double weight = algo::formula_p(0, (double)v.commits / present_tick, (double)present_tick, v.dee);
    e->weight = log(weight > 0 ? weight : DBL_EPSILON) + credibility;
    if (full_code) {
        *full_code = key.substr(0, separator_pos);
    }
    DLOG(INFO) << "text = '" << e->text << "', code_len = " << e->code.size() << ", weight = " << e->weight << ", commit_count = " << e->commit_count << ", present_tick = " << present_tick;
    return e;
}
