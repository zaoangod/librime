...
class Code : public vector<SyllableId> {
   public:
    Code() = default;
    Code(const Code::const_iterator& begin, const Code::const_iterator& end) : vector<SyllableId>(begin, end) {}
...
struct DictEntry {
    ...
    bool IsExactMatch() const { return matching_code_size == 0 || matching_code_size == code.size(); }
    bool IsPredictiveMatch() const { return matching_code_size != 0 && matching_code_size < code.size(); }
