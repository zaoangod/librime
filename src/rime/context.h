...
class RIME_DLL Context {
   public:
    ...
    using PropertyUpdateNotifier = signal<void(Context* ctx, const string& property)>;
    using KeyEventNotifier = signal<void(Context* ctx, const KeyEvent& key_event)>;
    ...
    OptionUpdateNotifier& option_update_notifier() { return option_update_notifier_; }
    PropertyUpdateNotifier& property_update_notifier() { return property_update_notifier_; }
