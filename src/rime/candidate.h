...
class Candidate {
   public:
    Candidate() = default;
    Candidate(const string& type, size_t start, size_t end, double quality = 0.) : type_(type), start_(start), end_(end), quality_(quality) {}
...
class SimpleCandidate : public Candidate {
   public:
    ...
    SimpleCandidate(const string& type, size_t start, size_t end, const string& text, const string& comment = string(), const string& preedit = string()) : Candidate(type, start, end), text_(text), comment_(comment), preedit_(preedit) {}
...
class ShadowCandidate : public Candidate {
   public:
    ShadowCandidate(const an<Candidate>& item, const string& type, const string& text = string(), const string& comment = string(), const bool inherit_comment = true) : Candidate(type, item->start(), item->end(), item->quality()), text_(text), comment_(comment), item_(item), inherit_comment_(inherit_comment) {}

    const string& text() const { return text_.empty() ? item_->text() : text_; }
    string comment() const { return inherit_comment_ && comment_.empty() ? item_->comment() : comment_; }
...
class UniquifiedCandidate : public Candidate {
   public:
    UniquifiedCandidate(const an<Candidate>& item, const string& type, const string& text = string(), const string& comment = string()) : Candidate(type, item->start(), item->end(), item->quality()), text_(text), comment_(comment) { Append(item); }

    const string& text() const { return text_.empty() && !items_.empty() ? items_.front()->text() : text_; }
    string comment() const { return comment_.empty() && !items_.empty() ? items_.front()->comment() : comment_; }
    string preedit() const { return !items_.empty() ? items_.front()->preedit() : string(); }
