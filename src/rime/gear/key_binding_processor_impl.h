...
const typename KeyBindingProcessor<T, N>::ActionDef KeyBindingProcessor<T, N>::kActionNoop = {"noop", nullptr};

template <class T, int N>
ProcessResult KeyBindingProcessor<T, N>::ProcessKeyEvent(const KeyEvent& key_event, Context* ctx, int keymap_selector, int fallback_options) {
    ...
    if (key_event.shift()) {
        if ((fallback_options & ShiftAsControl) != 0) {
            KeyEvent shift_as_control{key_event.keycode(), (key_event.modifier() & ~kShiftMask) | kControlMask};
            if (Accept(shift_as_control, ctx, keymap)) {
                return kAccepted;
            }
        }
        if ((fallback_options & IgnoreShift) != 0) {
            KeyEvent ignore_shift{key_event.keycode(), key_event.modifier() & ~kShiftMask};
...
template <class T, int N>
typename KeyBindingProcessor<T, N>::Keymap& KeyBindingProcessor<T, N>::get_keymap(int keymap_selector) {
    DCHECK_LT(keymap_selector, N);
    return keymaps_[keymap_selector];
}

template <class T, int N>
bool KeyBindingProcessor<T, N>::Accept(const KeyEvent& key_event, Context* ctx, Keymap& keymap) {
...
template <class T, int N>
void KeyBindingProcessor<T, N>::Keymap::Bind(KeyEvent key_event, HandlerPtr action) {
    if (action) {
...
template <class T, int N>
void KeyBindingProcessor<T, N>::LoadConfig(Config* config, const string& section, int keymap_selector) {
    auto& keymap = get_keymap(keymap_selector);
