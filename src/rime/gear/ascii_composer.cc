...
} ascii_mode_switch_styles[] = {{"inline_ascii", kAsciiModeSwitchInline}, {"commit_text", kAsciiModeSwitchCommitText}, {"commit_code", kAsciiModeSwitchCommitCode}, {"clear", kAsciiModeSwitchClear}, {"set_ascii_mode", kAsciiModeSet}, {"unset_ascii_mode", kAsciiModeUnset}, {NULL, kAsciiModeSwitchNoop}};

static void load_bindings(const an<ConfigMap>& src, AsciiModeSwitchKeyBindings* dest) {
    if (!src)
        return;
...
ProcessResult AsciiComposer::ProcessKeyEvent(const KeyEvent& key_event) {
    if (key_event.shift() + key_event.ctrl() + key_event.alt() + key_event.super() > 1) {
        shift_key_pressed_ = ctrl_key_pressed_ = alt_key_pressed_ = super_key_pressed_ = false;
        return kNoop;
    }
    ...
    if (is_shift || is_ctrl || is_alt || is_super) {
        if (key_event.release()) {
            if (shift_key_pressed_ || ctrl_key_pressed_ || alt_key_pressed_ || super_key_pressed_) {
                auto now = std::chrono::steady_clock::now();
                if (((is_shift && shift_key_pressed_) || (is_ctrl && ctrl_key_pressed_) || (is_alt && alt_key_pressed_) || (is_super && super_key_pressed_)) && now < toggle_expired_) {
                    ToggleAsciiModeWithKey(ch);
                }
                shift_key_pressed_ = ctrl_key_pressed_ = alt_key_pressed_ = super_key_pressed_ = false;
                return kNoop;
            }
        } else if (!(shift_key_pressed_ || ctrl_key_pressed_ || alt_key_pressed_ || super_key_pressed_)) {
            // first key down
            if (is_shift)
    ...
    if (key_event.ctrl() || key_event.alt() || key_event.super() || (key_event.shift() && ch == XK_space)) {
        return kNoop;
    }
...
ProcessResult AsciiComposer::ProcessCapsLock(const KeyEvent& key_event) {
    ...
    if (key_event.caps()) {
        if (!good_old_caps_lock_ && !key_event.release() && !key_event.ctrl() && !key_event.alt() && !key_event.super() && isascii(ch) && isalpha(ch)) {
...
void AsciiComposer::LoadConfig(Schema* schema) {
    ...
    if (!config->GetBool("ascii_composer/good_old_caps_lock", &good_old_caps_lock_)) {
        if (preset_config) {
            preset_config->GetBool("ascii_composer/good_old_caps_lock", &good_old_caps_lock_);
        }
    }
    if (auto bindings = config->GetMap("ascii_composer/switch_key")) {
        load_bindings(bindings, &bindings_);
    } else if (auto bindings = preset_config ? preset_config->GetMap("ascii_composer/switch_key") : nullptr) {
        load_bindings(bindings, &bindings_);
    } else {
    ...
    if (it != bindings_.end()) {
        ...
        if (caps_lock_switch_style_ == kAsciiModeSwitchInline || caps_lock_switch_style_ == kAsciiModeSet || caps_lock_switch_style_ == kAsciiModeUnset) {
            // can't do that
...
bool AsciiComposer::ToggleAsciiModeWithKey(int key_code) {
    ...
    bool new_mode = (style == kAsciiModeSet) ? true : (style == kAsciiModeUnset) ? false : !old_mode;
...
void AsciiComposer::SwitchAsciiMode(bool ascii_mode, AsciiModeSwitchStyle style) {
    ...
    if (ctx->IsComposing()) {
        ...
        if (style == kAsciiModeSwitchInline) {
            LOG(INFO) << "converting current composition to " << (ascii_mode ? "ascii" : "non-ascii") << " mode.";
            if (ascii_mode) {
                connection_ = ctx->update_notifier().connect([this](Context* ctx) { OnContextUpdate(ctx); });
            }
            ...
            ctx->Commit();
        } else if (style == kAsciiModeSwitchClear || style == kAsciiModeSet || style == kAsciiModeUnset) {
            ctx->Clear();
        }
    }
