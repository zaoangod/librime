...
FallbackSegmentor::FallbackSegmentor(const Ticket& ticket) : Segmentor(ticket) {}
...
bool FallbackSegmentor::Proceed(Segmentation* segmentation) {
    ...
    if (!segmentation->empty() && segmentation->back().start == segmentation->back().end) {
    ...
    if (!segmentation->empty()) {
        ...
        if (last.HasTag("raw")) {
            last.end = k + 1;
            DLOG(INFO) << "extend previous raw segment to [" << last.start << ", " << last.end << ")";
