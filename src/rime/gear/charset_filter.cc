...
CharsetFilterTranslation::CharsetFilterTranslation(an<Translation> translation) : translation_(translation) {
    LocateNextCandidate();
}
...
CharsetFilter::CharsetFilter(const Ticket& ticket) : Filter(ticket), TagMatching(ticket) {}

an<Translation> CharsetFilter::Apply(an<Translation> translation, CandidateList* candidates) {
