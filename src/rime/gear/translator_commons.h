...
class Phrase : public Candidate {
   public:
    Phrase(const Language* language, const string& type, size_t start, size_t end, const an<DictEntry>& entry) : Candidate(type, start, end), language_(language), entry_(entry) {}
    ...
    void set_syllabifier(an<PhraseSyllabifier> syllabifier) { syllabifier_ = syllabifier; }
    ...
    size_t matching_code_size() const { return entry_->matching_code_size != 0 ? entry_->matching_code_size : entry_->code.size(); }
    bool is_exact_match() const { return entry_->matching_code_size == 0 || entry_->matching_code_size == entry_->code.size(); }
    bool is_predicitve_match() const { return entry_->matching_code_size != 0 && entry_->matching_code_size < entry_->code.size(); }
    Code matching_code() const { return entry_->matching_code_size == 0 ? entry_->code : Code(entry_->code.begin(), entry_->code.begin() + entry_->matching_code_size); }
    Spans spans() { return syllabifier_ ? syllabifier_->Syllabify(this) : Spans(); }
...
class Sentence : public Phrase {
   public:
    Sentence(const Language* language) : Phrase(language, "sentence", 0, 0, New<DictEntry>()) {}
    Sentence(const Sentence& other) : Phrase(other), components_(other.components_), word_lengths_(other.word_lengths_) { entry_ = New<DictEntry>(other.entry()); }
...
class TranslatorOptions {
   public:
    ...
    void set_contextual_suggestions(bool enabled) { contextual_suggestions_ = enabled; }
