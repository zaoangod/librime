...
static bool is_after_digit_separator(Context* ctx) {
    ...
    return !comp.empty() && comp[0].HasTag("punct_number") && comp[0].length == ctx->input().length();
}

ProcessResult Punctuator::ProcessKeyEvent(const KeyEvent& key_event) {
    if (key_event.release() || key_event.ctrl() || key_event.alt() || key_event.super())
        return kNoop;
    ...
    if (ReconvertDigitSeparatorAsPunct(key) || ctx->PushInput(ch)) {
        if (punctuation_is_translated(ctx, "punct")) {
            ConfirmUniquePunct(punct_definition) || AutoCommitPunct(punct_definition) || PairPunct(punct_definition);
        }
    }
    return kAccepted;
}
...
bool Punctuator::ConvertDigitSeparator(char ch) {
    ...
    if (ctx->composition().empty() && is_after_number(ctx)) {
        ...
        ctx->PushInput(ch) && punctuation_is_translated(ctx, "punct_number") && (config_.digit_separator_commit() ? ctx->Commit() : ctx->composition().Forward());
        return true;
    }
    return false;
}
...
bool Punctuator::AlternatePunct(const string& key, const an<ConfigItem>& definition) {
    ...
    if (segment.status > Segment::kVoid && segment.HasTag("punct") && key == ctx->input().substr(segment.start, segment.end - segment.start)) {
...
bool PunctSegmentor::Proceed(Segmentation* segmentation) {
        ...
        return true;
    {
        ...
        DLOG(INFO) << "add a punctuation segment [" << segment.start << ", " << segment.end << ")";
        if (k == 0 && config_.is_digit_separator(ch) && is_after_number(engine_->context())) {
...
PunctTranslator::PunctTranslator(const Ticket& ticket) : Translator(ticket), formatter_(ticket) {
...
an<Candidate> CreatePunctCandidate(const string& punct, const Segment& segment) {
    ...
    return New<SimpleCandidate>("punct", segment.start, segment.end, punct, (is_half_shape ? half_shape : is_full_shape ? full_shape : ""), one_key ? punct : "");
}

an<Translation> PunctTranslator::Query(const string& input, const Segment& segment) {
    if (segment.HasTag("punct_number")) {
        if (!input.empty()) {
            string punct = input;
    ...
    auto translation = TranslateUniquePunct(input, segment, As<ConfigValue>(definition));
    if (!translation)
...
an<Translation> PunctTranslator::TranslateUniquePunct(const string& key, const Segment& segment, const an<ConfigValue>& definition) {
    if (!definition)
        return nullptr;
    return New<UniqueTranslation>(CreatePunctCandidate(definition->str(), segment));
}

an<Translation> PunctTranslator::TranslateAlternatingPunct(const string& key, const Segment& segment, const an<ConfigList>& definition) {
    if (!definition)
        return nullptr;
...
an<Translation> PunctTranslator::TranslateAutoCommitPunct(const string& key, const Segment& segment, const an<ConfigMap>& definition) {
...
an<Translation> PunctTranslator::TranslatePairedPunct(const string& key, const Segment& segment, const an<ConfigMap>& definition) {
