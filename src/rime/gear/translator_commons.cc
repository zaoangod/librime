...
void Sentence::Extend(const DictEntry& another, size_t end_pos, double new_weight) {
    entry_->weight = new_weight;
    entry_->text.append(another.text);
    entry_->code.insert(entry_->code.end(), another.code.begin(), another.code.end());
    components_.push_back(another);
    word_lengths_.push_back(end_pos - end());
    set_end(end_pos);
    DLOG(INFO) << "extend sentence " << end_pos << ") " << text() << " weight: " << weight();
}
...
TranslatorOptions::TranslatorOptions(const Ticket& ticket) {
    ...
    if (Config* config = ticket.schema->config()) {
        config->GetString(ticket.name_space + "/delimiter", &delimiters_) || config->GetString("speller/delimiter", &delimiters_);
        config->GetBool(ticket.name_space + "/contextual_suggestions", &contextual_suggestions_);
        config->GetBool(ticket.name_space + "/enable_completion", &enable_completion_);
        config->GetBool(ticket.name_space + "/strict_spelling", &strict_spelling_);
        config->GetDouble(ticket.name_space + "/initial_quality", &initial_quality_);
        preedit_formatter_.Load(config->GetList(ticket.name_space + "/preedit_format"));
        comment_formatter_.Load(config->GetList(ticket.name_space + "/comment_format"));
        user_dict_disabling_patterns_.Load(config->GetList(ticket.name_space + "/disable_user_dict_for_patterns"));
        string tag;
        ...
        // blacklist
        if (auto blacklist = config->GetList(ticket.name_space + "/dictionary_exclude")) {
