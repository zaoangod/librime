...
UnityTableEncoder::UnityTableEncoder(UserDictionary* user_dict) : TableEncoder(NULL), user_dict_(user_dict) {
    set_collector(this);
}
...
void UnityTableEncoder::CreateEntry(const string& word, const string& code_str, const string& weight_str) {
    if (!user_dict_)
        return;
    DictEntry entry;
    entry.text = word;
...
bool UnityTableEncoder::TranslateWord(const string& word, vector<string>* code) {
    if (!rev_dict_) {
        return false;
    }
    string str_list;
    if (rev_dict_->LookupStems(word, &str_list) || rev_dict_->ReverseLookup(word, &str_list)) {
...
size_t UnityTableEncoder::LookupPhrases(UserDictEntryIterator* result, const string& input, bool predictive, size_t limit, string* resume_key) {
    if (!user_dict_)
        return 0;
    return user_dict_->LookupWords(result, kEncodedPrefix + input, predictive, limit, resume_key);
}
