...
Memory::Memory(const Ticket& ticket) {
    ...
    language_.reset(user_dict_ ? new Language{user_dict_->name()} : dict_ ? new Language{Language::get_language_component(dict_->name())} : nullptr);

    Context* ctx = ticket.engine->context();
    commit_connection_ = ctx->commit_notifier().connect([this](Context* ctx) { OnCommit(ctx); });
    delete_connection_ = ctx->delete_notifier().connect([this](Context* ctx) { OnDeleteEntry(ctx); });
...
bool Memory::ProcessSegmentOnCommit(CommitEntry& commit_entry, const Segment& seg) {
...
void Memory::OnDeleteEntry(Context* ctx) {
    if (!user_dict_ || user_dict_->readonly() || !ctx || !ctx->HasMenu())
        return;
    auto phrase = As<Phrase>(Candidate::GetGenuineCandidate(ctx->GetSelectedCandidate()));
