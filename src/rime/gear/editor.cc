...
} editor_char_handler_definitions[] = {{"direct_commit", &Editor::DirectCommit}, {"add_to_input", &Editor::AddToInput}, {"noop", nullptr}};

Editor::Editor(const Ticket& ticket, bool auto_commit) : Processor(ticket), KeyBindingProcessor(editor_action_definitions) {
...
ProcessResult Editor::ProcessKeyEvent(const KeyEvent& key_event) {
    ...
    if (ctx->IsComposing()) {
        auto result = KeyBindingProcessor::ProcessKeyEvent(key_event, ctx, 0, FallbackOptions::All);
        if (result != kNoop) {
            return result;
        }
    }
    if (char_handler_ && !key_event.ctrl() && !key_event.alt() && !key_event.super() && ch > 0x20 && ch < 0x7f) {
        DLOG(INFO) << "input char: '" << (char)ch << "', " << ch << ", '" << key_event.repr() << "'";
...
bool Editor::RevertLastEdit(Context* ctx) {
    ...
    ctx->ReopenPreviousSelection() || (ctx->PopInput() && ctx->ReopenPreviousSegment());
    return true;
}

bool Editor::BackToPreviousInput(Context* ctx) {
    ctx->ReopenPreviousSegment() || ctx->ReopenPreviousSelection() || ctx->PopInput();
    return true;
}
...
bool Editor::BackToPreviousSyllable(Context* ctx) {
    ctx->ReopenPreviousSelection() || ((pop_input_by_syllable(ctx) || ctx->PopInput()) && ctx->ReopenPreviousSegment());
    return true;
}
