...
TableTranslation::TableTranslation(TranslatorOptions* options, const Language* language, const string& input, size_t start, size_t end, const string& preedit, DictEntryIterator&& iter, UserDictEntryIterator&& uter) : options_(options), language_(language), input_(input), start_(start), end_(end), preedit_(preedit), iter_(std::move(iter)), uter_(std::move(uter)) {
    if (options_)
...
an<Candidate> TableTranslation::Peek() {
    ...
    if (phrase) {
        ...
        phrase->set_quality(std::exp(e->weight) + options_->initial_quality() + (incomplete ? -1 : 0) + (is_user_phrase ? 0.5 : 0));
    }
    return phrase;
}
...
bool TableTranslation::PreferUserPhrase() {
    ...
    if (iter_.exhausted())
        return true;
    if (iter_.Peek()->remaining_code_length == 0 && (uter_.Peek()->remaining_code_length != 0 || is_constructed(uter_.Peek().get())))
        return false;
    else
        return true;
}
...
LazyTableTranslation::LazyTableTranslation(TableTranslator* translator, const string& input, size_t start, size_t end, const string& preedit, bool enable_user_dict) : TableTranslation(translator, translator->language(), input, start, end, preedit), dict_(translator->dict()), blacklist_(&translator->blacklist()), user_dict_(enable_user_dict ? translator->user_dict() : NULL), limit_(kInitialSearchLimit), user_dict_limit_(kInitialSearchLimit) {
...
bool LazyTableTranslation::FetchMoreUserPhrases() {
    if (!user_dict_ || user_dict_limit_ == 0)
        return false;
    size_t count = user_dict_->LookupWords(&uter_, input_, true, user_dict_limit_, &user_dict_key_);
...
bool LazyTableTranslation::FetchMoreTableEntries() {
    ...
    DLOG(INFO) << "fetching more table entries: limit = " << limit_ << ", count = " << previous_entry_count;
...
TableTranslator::TableTranslator(const Ticket& ticket) : Translator(ticket), Memory(ticket), TranslatorOptions(ticket) {
    if (!engine_)
        return;
    if (Config* config = engine_->schema()->config()) {
        config->GetBool(name_space_ + "/enable_charset_filter", &enable_charset_filter_);
        config->GetBool(name_space_ + "/enable_sentence", &enable_sentence_);
        config->GetBool(name_space_ + "/sentence_over_completion", &sentence_over_completion_);
        config->GetBool(name_space_ + "/enable_encoder", &enable_encoder_);
        config->GetBool(name_space_ + "/encode_commit_history", &encode_commit_history_);
        config->GetInt(name_space_ + "/max_phrase_length", &max_phrase_length_);
        config->GetInt(name_space_ + "/max_homographs", &max_homographs_);
        if (enable_sentence_ || sentence_over_completion_ || contextual_suggestions_) {
...
an<Translation> TableTranslator::Query(const string& input, const Segment& segment) {
    ...
    if (enable_completion_) {
        translation = Cached<LazyTableTranslation>(this, code, segment.start, segment.start + input.length(), preedit, enable_user_dict);
    } else {
        ...
        if (!iter.exhausted() || !uter.exhausted())
            translation = Cached<TableTranslation>(this, language(), code, segment.start, segment.start + input.length(), preedit, std::move(iter), std::move(uter));
    }
    if (translation) {
        bool filter_by_charset = enable_charset_filter_ && !engine_->context()->get_option("extended_charset");
        ...
        if (encode_commit_history_) {
            ...
            if (!history.empty()) {
                ...
                for (; it != history.rend(); ++it) {
                    if (it->type != "table" && it->type != "user_table" && it->type != "sentence" && it->type != "uniquified")
                        break;
                    if (phrase.empty()) {
...
SentenceTranslation::SentenceTranslation(TableTranslator* translator, an<Sentence>&& sentence, DictEntryCollector&& collector, UserDictEntryCollector&& ucollector, const string& input, size_t start) : translator_(translator), sentence_(std::move(sentence)), collector_(std::move(collector)), user_phrase_collector_(std::move(ucollector)), input_(input), start_(start) {
    PrepareSentence();
    CheckEmpty();
}
...
an<Candidate> SentenceTranslation::Peek() {
    ...
    auto result = New<Phrase>(translator_ ? translator_->language() : NULL, is_user_phrase ? "user_table" : "table", start_, start_ + code_length, entry);
    if (translator_) {
...
bool SentenceTranslation::CheckEmpty() {
    set_exhausted(!sentence_ && collector_.empty() && user_phrase_collector_.empty());
    return exhausted();
}
...
bool SentenceTranslation::PreferUserPhrase() const {
    ...
    if (user_phrase_code_length > 0 && user_phrase_code_length >= table_code_length) {
        return true;
    }
    return false;
}

inline static size_t consume_trailing_delimiters(size_t pos, const string& input, const string& delimiters) {
...
template <class Iter>
inline static void collect_entries(DictEntryList& entries, Iter& iter, int max_entries) {
...
an<Translation> TableTranslator::MakeSentence(const string& input, size_t start, bool include_prefix_phrases) {
    bool filter_by_charset = enable_charset_filter_ && !engine_->context()->get_option("extended_charset");
    ...
    for (size_t start_pos = 0; start_pos < input.length(); ++start_pos) {
        ...
        if (user_dict_ && user_dict_->loaded()) {
            for (size_t len = 1; len <= active_input.length(); ++len) {
                size_t consumed_length = consume_trailing_delimiters(len, active_input, delimiters_);
                ...
                if (!uter.exhausted()) {
                    ...
                    if (include_prefix_phrases && start_pos == 0) {
                        ...
                        DLOG(INFO) << "user phrase[" << consumed_length << "] cached: " << user_phrase_collector[consumed_length].cache_size();
                    }
                }
                if (resume_key > active_key && !boost::starts_with(resume_key, active_key))
                    break;
            }
        }
        if (encoder_ && encoder_->loaded()) {
            ...
            for (size_t len = 1; len <= active_input.length(); ++len) {
                ...
                if (!uter.exhausted()) {
                    ...
                    if (include_prefix_phrases && start_pos == 0) {
                        ...
                        // uter must not be consumed
                        user_phrase_collector[consumed_length] = std::move(uter);
                        DLOG(INFO) << "unity phrase[" << consumed_length << "] cached: " << user_phrase_collector[consumed_length].cache_size();
                    }
                }
                if (resume_key > active_key && !boost::starts_with(resume_key, active_key))
                    break;
        ...
        if (dict_ && dict_->loaded()) {
            ...
            for (const auto& m : boost::adaptors::reverse(matches)) {
                if (m.length == 0)
                    continue;
                size_t consumed_length = consume_trailing_delimiters(m.length, active_input, delimiters_);
                size_t end_pos = start_pos + consumed_length;
                auto& homographs = same_start_pos[end_pos];
                if (homographs.size() >= max_homographs_)
                    continue;
                DictEntryIterator iter;
                dict_->LookupWords(&iter, active_input.substr(0, m.length), false, 0, &blacklist());
                if (filter_by_charset) {
                ...
                if (!iter.exhausted()) {
                    ...
                    if (include_prefix_phrases && start_pos == 0) {
                        ...
                        DLOG(INFO) << "table[" << consumed_length << "]: " << collector[consumed_length].entry_count();
                    }
                }
            }
        }
    ...
    if (auto sentence = poet_->MakeSentence(graph, input.length(), GetPrecedingText(start))) {
        auto result = Cached<SentenceTranslation>(this, std::move(sentence), std::move(collector), std::move(user_phrase_collector), input, start);
