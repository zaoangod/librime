...
class ReverseLookupTranslation : public TableTranslation {
   public:
    ReverseLookupTranslation(ReverseLookupDictionary* dict, TranslatorOptions* options, const string& input, size_t start, size_t end, const string& preedit, DictEntryIterator&& iter, bool quality) : TableTranslation(options, NULL, input, start, end, preedit, std::move(iter)), dict_(dict), options_(options), quality_(quality) {}
...
an<Candidate> ReverseLookupTranslation::Peek() {
    ...
    an<Candidate> cand = New<SimpleCandidate>("reverse_lookup", start_, end_, entry->text, !tips.empty() ? tips : entry->comment, preedit_);
    return cand;
}

int ReverseLookupTranslation::Compare(an<Translation> other, const CandidateList& candidates) {
...
ReverseLookupTranslator::ReverseLookupTranslator(const Ticket& ticket) : Translator(ticket), tag_("reverse_lookup") {
...
void ReverseLookupTranslator::Initialize() {
    ...
    auto rev_component = ReverseLookupDictionary::Require("reverse_lookup_dictionary");
    if (!rev_component)
        return;
...
an<Translation> ReverseLookupTranslator::Query(const string& input, const Segment& segment) {
    ...
    if (start < input.length()) {
        if (options_ && options_->enable_completion()) {
            ...
            size_t consumed = syllabifier.BuildSyllableGraph(code, *dict_->prism(), &graph);
            if (consumed == code.length()) {
                auto collector = dict_->Lookup(graph, 0);
                if (collector && !collector->empty() && collector->rbegin()->first == consumed) {
                    iter = std::move(collector->rbegin()->second);
                    quality = !graph.vertices.empty() && (graph.vertices.rbegin()->second == kNormalSpelling);
                }
            }
        }
    }
    if (!iter.exhausted()) {
        return Cached<ReverseLookupTranslation>(rev_dict_.get(), options_.get(), code, segment.start, segment.end, preedit, std::move(iter), quality);
    }
    return nullptr;
}
