...
static bool reached_max_code_length(const an<Candidate>& cand, int max_code_length) {
    if (!cand)
        return false;
...
static bool is_auto_selectable(const an<Candidate>& cand, const string& input, const string& delimiters) {
    return
        // reaches end of input
        cand->end() == input.length() && (is_table_entry(cand) || is_simple_candidate(cand)) &&
        // no delimiters
        input.find_first_of(delimiters, cand->start()) == string::npos;
}

static bool expecting_an_initial(Context* ctx, const string& alphabet, const string& finals) {
    size_t caret_pos = ctx->caret_pos();
    if (caret_pos == 0 || caret_pos == ctx->composition().GetCurrentStartPosition()) {
        return true;
    }
    const string& input(ctx->input());
    char previous_char = input[caret_pos - 1];
    return belongs_to(previous_char, finals) || !belongs_to(previous_char, alphabet);
}

Speller::Speller(const Ticket& ticket) : Processor(ticket), alphabet_(kRimeAlphabet) {
...
bool Speller::AutoSelectAtMaxCodeLength(Context* ctx) {
    ...
    if (cand && reached_max_code_length(cand, max_code_length_) && is_auto_selectable(cand, ctx->input(), delimiters_)) {
...
bool Speller::AutoSelectUniqueCandidate(Context* ctx) {
    ...
    if (auto_select_pattern_.empty()) {
        matches_input_pattern = max_code_length_ == 0 ||  // match any length if not set
...
bool Speller::AutoSelectPreviousMatch(Context* ctx, Segment* previous_segment) {
    ...
    if (is_auto_selectable(previous_segment->GetSelectedCandidate(), converted, delimiters_)) {
...
bool Speller::AutoClear(Context* ctx) {
    if (!ctx->HasMenu() && auto_clear_ > kClearNone && (auto_clear_ != kClearMaxLength || max_code_length_ == 0 || ctx->input().length() >= (size_t)max_code_length_)) {
        ctx->Clear();
        return true;
    }
    return false;
...
bool Speller::FindEarlierMatch(Context* ctx, size_t start, size_t end) {
    ...
    while (--end > start) {
        ...
        if (is_auto_selectable(segment.GetSelectedCandidate(), converted, delimiters_)) {
