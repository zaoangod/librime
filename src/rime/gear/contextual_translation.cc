...
bool ContextualTranslation::Replenish() {
    ...
    while (!translation_->exhausted() && cache_.size() + queue.size() < kContextualSearchLimit) {
        auto cand = translation_->Peek();
        DLOG(INFO) << cand->text() << " cache/queue: " << cache_.size() << "/" << queue.size();
        if (cand->type() == "phrase" || cand->type() == "user_phrase" || cand->type() == "table" || cand->type() == "user_table" || cand->type() == "completion") {
...
an<Phrase> ContextualTranslation::Evaluate(an<Phrase> phrase) {
    ...
    double weight = Grammar::Evaluate(preceding_text_, phrase->text(), phrase->weight(), is_rear, grammar_);
    phrase->set_weight(weight);
    DLOG(INFO) << "contextual suggestion: " << phrase->text() << " weight: " << phrase->weight();
    return phrase;
}
