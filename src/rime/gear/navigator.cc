...
static Navigator::ActionDef navigation_actions[] = {{"rewind", &Navigator::Rewind}, {"left_by_char", &Navigator::LeftByChar}, {"right_by_char", &Navigator::RightByChar}, {"left_by_syllable", &Navigator::LeftBySyllable}, {"right_by_syllable", &Navigator::RightBySyllable}, {"home", &Navigator::Home}, {"end", &Navigator::End}, Navigator::kActionNoop};

Navigator::Navigator(const Ticket& ticket) : Processor(ticket), KeyBindingProcessor(navigation_actions) {
    ...
    select_connection_ = engine_->context()->select_notifier().connect([this](Context* ctx) { OnSelect(ctx); });
}
...
ProcessResult Navigator::ProcessKeyEvent(const KeyEvent& key_event) {
    ...
    if (!ctx->IsComposing())
        return kNoop;
    TextOrientation text_orientation = ctx->get_option("_vertical") ? Vertical : Horizontal;
    return KeyBindingProcessor::ProcessKeyEvent(key_event, ctx, text_orientation, FallbackOptions::All);
}
...
bool Navigator::Rewind(Context* ctx) {
    ...
    (spans_.Count() > 1 && spans_.HasVertex(ctx->caret_pos()) ? JumpLeft(ctx) : MoveLeft(ctx)) || GoToEnd(ctx);
    return true;
}
...
void Navigator::BeginMove(Context* ctx) {
    ...
    if (input_ != ctx->input() || ctx->caret_pos() > spans_.end()) {
        ...
        for (const auto& seg : ctx->composition()) {
            if (auto phrase = As<Phrase>(Candidate::GetGenuineCandidate(seg.GetSelectedCandidate()))) {
