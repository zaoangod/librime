...
struct SyllabifyTask {
    ...
    size_t target_pos;
    function<void(SyllabifyTask* task, size_t depth, size_t current_pos, size_t next_pos)> push;
    function<void(SyllabifyTask* task, size_t depth)> pop;
};

static bool syllabify_dfs(SyllabifyTask* task, size_t depth, size_t current_pos) {
...
class ScriptSyllabifier : public PhraseSyllabifier {
   public:
    ScriptSyllabifier(ScriptTranslator* translator, Corrector* corrector, const string& input, size_t start) : translator_(translator), input_(input), start_(start), syllabifier_(translator->delimiters(), translator->enable_completion(), translator->strict_spelling()) {
        if (corrector) {
...
class ScriptTranslation : public Translation {
   public:
    ScriptTranslation(ScriptTranslator* translator, Corrector* corrector, Poet* poet, const string& input, size_t start, size_t end_of_input) : translator_(translator), poet_(poet), start_(start), end_of_input_(end_of_input), syllabifier_(New<ScriptSyllabifier>(translator, corrector, input, start)), enable_correction_(corrector) { set_exhausted(true); }
   ...
   protected:
    ...
    void EnrollEntries(map<int, DictEntryList>& entries_by_end_pos, const an<QueryResult>& query_result);
...
ScriptTranslator::ScriptTranslator(const Ticket& ticket) : Translator(ticket), Memory(ticket), TranslatorOptions(ticket) {
    if (!engine_)
        return;
    if (Config* config = engine_->schema()->config()) {
        ...
        if (!config->GetBool(name_space_ + "/enable_word_completion", &enable_word_completion_)) {
...
an<Translation> ScriptTranslator::Query(const string& input, const Segment& segment) {
    ...
    bool enable_user_dict = user_dict_ && user_dict_->loaded() && !IsUserDictDisabledFor(input);

    size_t end_of_input = engine_->context()->input().length();
    // the translator should survive translations it creates
    auto result = New<ScriptTranslation>(this, corrector_.get(), poet_.get(), input, segment.start, end_of_input);
    if (!result || !result->Evaluate(dict_.get(), enable_user_dict ? user_dict_.get() : NULL)) {
        return nullptr;
    }
...
bool ScriptTranslator::ConcatenatePhrases(CommitEntry& commit_entry, const vector<an<Phrase>>& phrases) {
...
bool ScriptTranslator::ProcessSegmentOnCommit(CommitEntry& commit_entry, const Segment& seg) {
...
Spans ScriptSyllabifier::Syllabify(const Phrase* phrase) {
    ...
    SyllabifyTask task{phrase->code(), syllable_graph_, phrase->end() - start_, [&](SyllabifyTask* task, size_t depth, size_t current_pos, size_t next_pos) { vertices.push_back(start_ + next_pos); }, [&](SyllabifyTask* task, size_t depth) { vertices.pop_back(); }};
...
size_t ScriptSyllabifier::BuildSyllableGraph(Prism& prism) {
    return (size_t)syllabifier_.BuildSyllableGraph(input_, prism, &syllable_graph_);
}
...
string ScriptSyllabifier::GetPreeditString(const Phrase& cand) const {
    ...
    SyllabifyTask task{cand.matching_code(), syllable_graph_, cand.end() - start_,
                       [&](SyllabifyTask* task, size_t depth, size_t current_pos, size_t next_pos) {
                           ...
                           if (depth > 0 && len > 0 && delimiters.find(output[len - 1]) == string::npos) {
...
string ScriptSyllabifier::GetOriginalSpelling(const Phrase& cand) const {
    if (translator_ && static_cast<int>(cand.code().size()) <= translator_->spelling_hints()) {
...
static bool has_exact_match_phrase(Ptr ptr, Iter iter, size_t consumed) {
    return ptr && iter->first == consumed && !iter->second.exhausted() && iter->second.Peek()->IsExactMatch();
}
...
bool ScriptTranslation::Evaluate(Dictionary* dict, UserDictionary* user_dict) {
    ...
    bool predict_word = translator_->enable_word_completion() && start_ + consumed == end_of_input_;

    phrase_ = dict->Lookup(syllable_graph, 0, &translator_->blacklist(), predict_word);
    if (user_dict) {
        const size_t kUnlimitedDepth = 0;
        const size_t kNumSyllablesToPredictWord = 4;
        user_phrase_ = user_dict->Lookup(syllable_graph, 0, kUnlimitedDepth, predict_word ? kNumSyllablesToPredictWord : 0);
    }
    ...
    if (has_at_least_two_syllables && !has_exact_match_phrase(phrase_, phrase_iter_, consumed) && !has_exact_match_phrase(user_phrase_, user_phrase_iter_, consumed)) {
...
bool ScriptTranslation::Next() {
    ...
    } while (enable_correction_ && syllabifier_->IsCandidateCorrection(*candidate_) &&
...
bool ScriptTranslation::IsNormalSpelling() const {
    const auto& syllable_graph = syllabifier_->syllable_graph();
    return !syllable_graph.vertices.empty() && (syllable_graph.vertices.rbegin()->second == kNormalSpelling);
}
...
an<Candidate> ScriptTranslation::Peek() {
    ...
    if (candidate_->comment().empty()) {
        ...
        if (!spelling.empty() && (translator_->always_show_comments() || spelling != candidate_->preedit())) {
...
template <typename T>
inline static bool prefer_user_phrase(T user_phrase_weight, T sys_phrase_weight, function<bool()> compare_on_tie = always_true) {
    return user_phrase_weight > sys_phrase_weight || (user_phrase_weight == sys_phrase_weight && compare_on_tie());
}
...
bool ScriptTranslation::PrepareCandidate() {
    ...
    if (user_phrase_code_length > 0 && prefer_user_phrase(user_phrase_code_length, phrase_code_length, [this]() {
            const int kNumExactMatchOnTop = 1;
            size_t full_code_length = end_of_input_ - start_;
            return candidate_index_ >= kNumExactMatchOnTop || prefer_user_phrase(has_exact_match_phrase(user_phrase_, user_phrase_iter_, full_code_length), has_exact_match_phrase(phrase_, phrase_iter_, full_code_length));
        })) {
        UserDictEntryIterator& uter = user_phrase_iter_->second;
        const auto& entry = uter.Peek();
        DLOG(INFO) << "user phrase '" << entry->text << "', code length: " << user_phrase_code_length;
        candidate_source_ = kUserPhrase;
        candidate_ = New<Phrase>(translator_->language(), entry->IsPredictiveMatch() ? "completion" : "user_phrase", start_, start_ + user_phrase_code_length, entry);
        candidate_->set_quality(std::exp(entry->weight) + translator_->initial_quality() + (IsNormalSpelling() ? 0.5 : -0.5));
        return true;
    } else if (phrase_code_length > 0) {
        DictEntryIterator& iter = phrase_iter_->second;
        const auto& entry = iter.Peek();
        DLOG(INFO) << "phrase '" << entry->text << "', code length: " << phrase_code_length;
        candidate_source_ = kSysPhrase;
        candidate_ = New<Phrase>(translator_->language(), entry->IsPredictiveMatch() ? "completion" : "phrase", start_, start_ + phrase_code_length, entry);
        candidate_->set_quality(std::exp(entry->weight) + translator_->initial_quality() + (IsNormalSpelling() ? 0 : -1));
        return true;
    } else {
        candidate_source_ = kUninitialized;
        candidate_ = nullptr;
...
bool ScriptTranslation::CheckEmpty() {
    set_exhausted((!phrase_ || phrase_iter_ == phrase_->rend()) && (!user_phrase_ || user_phrase_iter_ == user_phrase_->rend()));
    return exhausted();
}

template <class QueryResult>
void ScriptTranslation::EnrollEntries(map<int, DictEntryList>& entries_by_end_pos, const an<QueryResult>& query_result) {
    if (query_result) {
        for (auto& y : *query_result) {
            DictEntryList& homophones = entries_by_end_pos[y.first];
            while (homophones.size() < translator_->max_homophones() && !y.second.exhausted()) {
...
an<Sentence> ScriptTranslation::MakeSentence(Dictionary* dict, UserDictionary* user_dict) {
    ...
    for (const auto& x : syllable_graph.edges) {
        ...
        if (user_dict) {
            EnrollEntries(same_start_pos, user_dict->Lookup(syllable_graph, x.first, kMaxSyllablesForUserPhraseQuery));
        }
        // merge lookup results
        EnrollEntries(same_start_pos, dict->Lookup(syllable_graph, x.first, &translator_->blacklist()));
    }
    if (auto sentence = poet_->MakeSentence(graph, syllable_graph.interpreted_length, translator_->GetPrecedingText(start_))) {
