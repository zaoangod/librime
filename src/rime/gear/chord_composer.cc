...
ChordComposer::ChordComposer(const Ticket& ticket) : Processor(ticket), KeyBindingProcessor<ChordComposer>(action_definitions) {
    if (!engine_)
        return;
    if (Config* config = engine_->schema()->config()) {
        ...
        config->GetBool("chord_composer/finish_chord_on_first_key_release", &finish_chord_on_first_key_release_);
    ...
    update_connection_ = ctx->update_notifier().connect([this](Context* ctx) { OnContextUpdate(ctx); });
    unhandled_key_connection_ = ctx->unhandled_key_notifier().connect([this](Context* ctx, const KeyEvent& key) { OnUnhandledKey(ctx, key); });
}
...
inline static int get_base_layer_key_code(const KeyEvent& key_event) {
    ...
    return (is_shift && ch >= 0x20 && ch <= 0x7e) ? map_to_base_layer[ch - 0x20] : ch;
}
...
bool ChordComposer::FinishChordConditionIsMet() const {
    return finish_chord_on_first_key_release_ || finish_chord_on_all_keys_released(state_);
}

ProcessResult ChordComposer::ProcessChordingKey(const KeyEvent& key_event) {
    if ((key_event.ctrl() && !use_control_) || (key_event.alt() && !use_alt_) || (key_event.shift() && !use_shift_) || (key_event.super() && !use_super_) || (key_event.caps() && !use_caps_)) {
        ClearChord();
        state_.Clear();
        return kNoop;
    }
    ...
    // non chording key
    if (std::find(chording_keys_.begin(), chording_keys_.end(), KeyEvent{ch, 0}) == chording_keys_.end()) {
        ClearChord();
        state_.Clear();
        return kNoop;
    }
    ...
    if (is_key_up) {
        ...
        if (chording_key_released && FinishChordConditionIsMet() && !state_.recognized_chord.empty()) {
...
void ChordComposer::OnUnhandledKey(Context* ctx, const KeyEvent& key) {
    ...
    if ((key.modifier() & ~kShiftMask) == 0 && key.keycode() >= 0x20 && key.keycode() <= 0x7e) {
        raw_sequence_.clear();
        DLOG(INFO) << "clear raw sequence.";
    }
}
