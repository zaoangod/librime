...
static struct KeyBindingConditionDef {
    ...
    const char* name;
} condition_definitions[] = {{kWhenPredicting, "predicting"}, {kWhenPaging, "paging"}, {kWhenHasMenu, "has_menu"}, {kWhenComposing, "composing"}, {kAlways, "always"}, {kNever, NULL}};
...
static void radio_select_option(Context* ctx, const Switches::SwitchOption& the_option) {
    Switches::FindRadioGroupOption(the_option.the_switch, [ctx, &the_option](Switches::SwitchOption option) {
...
static Switches::SwitchOption switch_by_index(Switches& switches, const string& option) {
    try {
...
static void toggle_option(Engine* engine, const string& option) {
    ...
    auto the_option = is_switch_index(option) ? switch_by_index(switches, option) : switches.OptionByName(option);
    if (the_option.found() && the_option.type == Switches::kRadioGroup) {
        auto selected_option = switches.FindRadioGroupOption(the_option.the_switch, [ctx](Switches::SwitchOption option) { return ctx->get_option(option.option_name) ? Switches::kFound : Switches::kContinue; });
...
void KeyBindings::LoadBindings(const an<ConfigList>& bindings) {
    ...
    for (size_t i = 0; i < bindings->size(); ++i) {
        ...
        if (!key.Parse(pattern->str())) {
            LOG(WARNING) << "invalid key binding #" << i << ", with invalid accept pattern: " << pattern->str() << ".";
            continue;
        }
        if (auto target = map->GetValue("send")) {
            ...
            if (key.Parse(target->str())) {
                ...
                LOG(WARNING) << "invalid key binding #" << i << ", with invalid send pattern: " << target->str() << ".";
                continue;
            }
        } else if (auto target = map->GetValue("send_sequence")) {
            if (!binding.target.Parse(target->str())) {
                LOG(WARNING) << "invalid key sequence #" << i << ", with invalid send_sequence pattern: " << target->str() << ".";
                continue;
            }
        } else if (auto option = map->GetValue("toggle")) {
            binding.action = [option](auto engine) { toggle_option(engine, option->str()); };
        } else if (auto option = map->GetValue("set_option")) {
            binding.action = [option](auto engine) { set_option(engine, option->str()); };
        } else if (auto option = map->GetValue("unset_option")) {
            binding.action = [option](auto engine) { unset_option(engine, option->str()); };
        } else if (auto schema = map->GetValue("select")) {
            binding.action = [schema](auto engine) { select_schema(engine, schema->str()); };
        } else {
            LOG(WARNING) << "invalid key binding #" << i << ", accept: " << pattern->str() << ", when: " << whence->str() << ".";
            continue;
        }
        Bind(key, binding);
    }
...
KeyBinder::KeyBinder(const Ticket& ticket) : Processor(ticket), key_bindings_(new KeyBindings), redirecting_(false), last_key_(0) {
    LoadConfig();
}
...
bool KeyBinder::ReinterpretPagingKey(const KeyEvent& key_event) {
    ...
    if (last_key_ == '.' && ch >= 'a' && ch <= 'z') {
        ...
        if (!input.empty() && input[input.length() - 1] != '.') {
            LOG(INFO) << "reinterpreted key: '" << last_key_ << "', successor: '" << (char)ch << "'";
