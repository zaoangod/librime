...
HistoryTranslator::HistoryTranslator(const Ticket& ticket) : Translator(ticket), tag_("abc"), size_(1), initial_quality_(1000) {
...
an<Translation> HistoryTranslator::Query(const string& input, const Segment& segment) {
    ...
    for (; it != history.rend(); ++it) {
        if (it->type == "thru")
            continue;
        auto candidate = New<SimpleCandidate>(it->type, segment.start, segment.end, it->text);
