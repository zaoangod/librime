...
struct Line {
    ...
    struct Components {
        ...
        Components(const Line* line) {
            for (const Line* cursor = line; !cursor->empty(); cursor = cursor->predecessor) {
    ...
    string context() const {
        // look back 2 words
        return empty() ? string() : !predecessor || predecessor->empty() ? last_word() : predecessor->last_word() + last_word();
    }
...
Poet::Poet(const Language* language, Config* config, Compare compare) : language_(language), grammar_(create_grammar(config)), compare_(compare) {}
...
bool Poet::LeftAssociateCompare(const Line& one, const Line& other) {
    ...
    if (one.weight == other.weight) {
        ...
        if (one_word_lens.size() > other_word_lens.size())
            return true;
        if (one_word_lens.size() == other_word_lens.size()) {
            return std::lexicographical_compare(one_word_lens.begin(), one_word_lens.end(), other_word_lens.begin(), other_word_lens.end());
        }
    }
    return false;
}
...
template <int N>
static vector<const Line*> find_top_candidates(const LineCandidates& candidates, Poet::Compare compare) {
    vector<const Line*> top;
    top.reserve(N + 1);
    for (const auto& candidate : candidates) {
        auto pos = std::upper_bound(top.begin(), top.end(), &candidate.second, [&](const Line* a, const Line* b) { return compare(*b, *a); });  // desc
        if (pos - top.begin() >= N)
            continue;
...
struct BeamSearch {
    ...
    static void Initiate(State& initial_state) { initial_state.emplace("", Line::kEmpty); }

    static void ForEachCandidate(const State& state, Poet::Compare compare, UpdateLineCandidate update) {
        auto top_candidates = find_top_candidates<kMaxLineCandidates>(state, compare);
...
struct DynamicProgramming {
    ...
    static void ForEachCandidate(const State& state, Poet::Compare compare, UpdateLineCandidate update) { update(state); }

    static Line& BestLineToUpdate(State& state, const Line& new_line) { return state; }

    static const Line& BestLineInState(const State& final_state, Poet::Compare compare) { return final_state; }
};

template <class Strategy>
an<Sentence> Poet::MakeSentenceWithStrategy(const WordGraph& graph, size_t total_length, const string& preceding_text) {
    ...
    for (const auto& sv : graph) {
        ...
        const auto update = [this, &states, &sv, start_pos, total_length, &preceding_text](const Line& candidate) {
            for (const auto& ev : sv.second) {
                ...
                for (const auto& entry : entries) {
                    const string& context = candidate.empty() ? preceding_text : candidate.context();
                    double weight = candidate.weight + Grammar::Evaluate(context, entry->text, entry->weight, is_rear, grammar_.get());
                    ...
                    if (best.empty() || compare_(best, new_line)) {
                        DLOG(INFO) << "updated line ending at " << end_pos << " with text: ..." << new_line.last_word() << " weight: " << new_line.weight;
                        best = new_line;
                    }
                }
            }
...
an<Sentence> Poet::MakeSentence(const WordGraph& graph, size_t total_length, const string& preceding_text) {
    return grammar_ ? MakeSentenceWithStrategy<BeamSearch>(graph, total_length, preceding_text) : MakeSentenceWithStrategy<DynamicProgramming>(graph, total_length, preceding_text);
}
