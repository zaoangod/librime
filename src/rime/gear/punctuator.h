...
class PunctConfig {
   public:
    ...
    bool is_digit_separator(char ch) const { return digit_separators_.find(ch) != string::npos; }
...
class PunctTranslator : public Translator {
   ...
   protected:
    an<Translation> TranslateUniquePunct(const string& key, const Segment& segment, const an<ConfigValue>& definition);
    an<Translation> TranslateAlternatingPunct(const string& key, const Segment& segment, const an<ConfigList>& definition);
    an<Translation> TranslateAutoCommitPunct(const string& key, const Segment& segment, const an<ConfigMap>& definition);
    an<Translation> TranslatePairedPunct(const string& key, const Segment& segment, const an<ConfigMap>& definition);
