...
class SchemaSelection : public SimpleCandidate, public SwitcherCommand {
   public:
    SchemaSelection(Schema* schema) : SimpleCandidate("schema", 0, 0, schema->schema_name()), SwitcherCommand(schema->schema_id()) {}
...
class SchemaAction : public ShadowCandidate, public SwitcherCommand {
   public:
    SchemaAction(an<Candidate> schema, an<Candidate> command) : ShadowCandidate(schema, command->type()), SwitcherCommand(As<SwitcherCommand>(schema)->keyword()), command_(As<SwitcherCommand>(command)) {}
    virtual void Apply(Switcher* switcher);
...
int SchemaListTranslation::Compare(an<Translation> other, const CandidateList& candidates) {
...
void SchemaListTranslation::LoadSchemaList(Switcher* switcher) {
    ...
    Switcher::ForEachSchemaListEntry(config, [this, current_schema, user_config, now](const string& schema_id) {
        if (current_schema && schema_id == current_schema->schema_id())
            return /* continue = */ true;
        Schema schema(schema_id);
        auto cand = New<SchemaSelection>(&schema);
        int timestamp = 0;
        if (user_config && user_config->GetInt("var/schema_access_time/" + schema_id, &timestamp)) {
            if (timestamp <= now)
    ...
    std::stable_sort(candies_.begin() + fixed, candies_.end(), [](const an<Candidate>& x, const an<Candidate>& y) { return x->quality() > y->quality(); });
}

SchemaListTranslator::SchemaListTranslator(const Ticket& ticket) : Translator(ticket) {}

an<Translation> SchemaListTranslator::Query(const string& input, const Segment& segment) {
