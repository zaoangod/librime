...
static void load_patterns(RecognizerPatterns* patterns, an<ConfigMap> map) {
    ...
    for (auto it = map->begin(); it != map->end(); ++it) {
        ...
        try {
            ...
            LOG(ERROR) << "error parsing pattern /" << value->str() << "/: " << e.what();
        }
    }
}
...
RecognizerMatch RecognizerPatterns::GetMatch(const string& input, const Segmentation& segmentation) const {
...
ProcessResult Recognizer::ProcessKeyEvent(const KeyEvent& key_event) {
    if (patterns_.empty() || key_event.ctrl() || key_event.alt() || key_event.super() || key_event.release()) {
        return kNoop;
    }
