...
inline static string get_state_label(const SwitchOption& option, size_t state_index, bool abbreviate = false) {
    return string(Switches::GetStateLabel(option.the_switch, state_index, abbreviate));
}

inline static bool has_state_label(const SwitchOption& option, size_t state_index) {
...
class Switch : public SimpleCandidate, public SwitcherCommand {
   public:
    Switch(const SwitchOption& option, bool current_state, bool auto_save) : SimpleCandidate("switch", 0, 0, get_state_label(option, current_state), kRightArrow + get_state_label(option, 1 - current_state)), SwitcherCommand(option.option_name), target_state_(!current_state), auto_save_(auto_save) {}
...
class RadioGroup : public std::enable_shared_from_this<RadioGroup> {
   public:
    RadioGroup(Context* context, Switcher* switcher) : context_(context), switcher_(switcher) {}
...
class RadioOption : public SimpleCandidate, public SwitcherCommand {
   public:
    RadioOption(an<RadioGroup> group, const string& state_label, const string& option_name) : SimpleCandidate("switch", 0, 0, state_label), SwitcherCommand(option_name), group_(group) {}
    void Apply(Switcher* switcher) override;
...
an<RadioOption> RadioGroup::CreateOption(const SwitchOption& option, size_t option_index) {
    auto radio_option = New<RadioOption>(shared_from_this(), get_state_label(option, option_index), option.option_name);
...
class FoldedOptions : public SimpleCandidate, public SwitcherCommand {
   public:
    FoldedOptions(Config* config) : SimpleCandidate("unfold", 0, 0, ""), SwitcherCommand("_fold_options") { LoadConfig(config); }
    void Apply(Switcher* switcher) override;
...
void SwitchTranslation::LoadSwitches(Switcher* switcher) {
    ...
    switches.FindOption([this, switcher, context, &groups](Switches::SwitchOption option) -> Switches::FindResult {
        ...
        if (option.type == Switches::kToggleOption) {
            ...
            Append(New<Switch>(option, current_state, switcher->IsAutoSave(option.option_name)));
    ...
    if (switcher->context()->get_option("_fold_options")) {
        ...
        switches.FindOption([context, &folded_options](Switches::SwitchOption option) -> Switches::FindResult {
            bool current_state = context->get_option(option.option_name);
            if (option.type == Switches::kToggleOption) {
...
an<Translation> SwitchTranslator::Query(const string& input, const Segment& segment) {
