...
class ScriptTranslator : public Translator, public Memory, public TranslatorOptions {
   public:
    ScriptTranslator(const Ticket& ticket);

    virtual an<Translation> Query(const string& input, const Segment& segment) override;
    virtual bool Memorize(const CommitEntry& commit_entry) override;
    virtual bool ProcessSegmentOnCommit(CommitEntry& commit_entry, const Segment& seg) override;
    ...
    bool ConcatenatePhrases(CommitEntry& commit_entry, const vector<an<Phrase>>& phrases);
