...
static Selector::ActionDef selector_actions[] = {
    {"previous_candidate", &Selector::PreviousCandidate}, {"next_candidate", &Selector::NextCandidate}, {"previous_page", &Selector::PreviousPage}, {"next_page", &Selector::NextPage}, {"home", &Selector::Home}, {"end", &Selector::End}, Selector::kActionNoop,
};

Selector::Selector(const Ticket& ticket) : Processor(ticket), KeyBindingProcessor(selector_actions) {
...
ProcessResult Selector::ProcessKeyEvent(const KeyEvent& key_event) {
    ...
    TextOrientation text_orientation = is_vertical_text(ctx) ? Vertical : Horizontal;
    CandidateListLayout candidate_list_layout = is_linear_layout(ctx) ? Linear : Stacked;
    auto result = KeyBindingProcessor::ProcessKeyEvent(key_event, ctx, text_orientation | candidate_list_layout, FallbackOptions::None);
    if (result != kNoop) {
        return result;
    }
