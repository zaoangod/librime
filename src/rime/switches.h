...
struct StringSlice {
    ...
    operator string() const { return str && length ? string(str, length) : string(); }
...
class Switches {
   ...
   private:
    SwitchOption FindOptionFromConfigItem(ConfigItemRef& item, size_t switch_index, function<FindResult(SwitchOption option)> callback);
