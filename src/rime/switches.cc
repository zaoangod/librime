...
Switches::SwitchOption Switches::FindOptionFromConfigItem(ConfigItemRef& item, size_t switch_index, function<FindResult(SwitchOption option)> callback) {
    auto the_switch = As<ConfigMap>(*item);
    auto name = item["name"];
    auto options = item["options"];
    if (name.IsValue()) {
        SwitchOption option{
            the_switch, kToggleOption, name.ToString(), reset_value(item), switch_index,
        };
        ...
        for (size_t option_index = 0; option_index < options.size(); ++option_index) {
            SwitchOption option{
                the_switch, kRadioGroup, options[option_index].ToString(), reset_value(item), switch_index, option_index,
            };
            if (callback(option) == kFound)
                return option;
        }
...
Switches::SwitchOption Switches::FindOption(function<FindResult(SwitchOption option)> callback) {
...
Switches::SwitchOption Switches::OptionByName(const string& option_name) {
    return FindOption([&option_name](SwitchOption option) { return option.option_name == option_name ? kFound : kContinue; });
}
...
Switches::SwitchOption Switches::Cycle(const SwitchOption& current) {
    if (auto options = As<ConfigList>(current.the_switch->Get("options"))) {
        ...
        if (next_option_index != current.option_index) {
            return {
                current.the_switch, current.type, options->GetValueAt(next_option_index)->str(), current.reset_value, current.switch_index, next_option_index,
            };
        }
    }
    return {};
...
Switches::SwitchOption Switches::Reset(const SwitchOption& current) {
    ...
    if (auto options = As<ConfigList>(current.the_switch->Get("options"))) {
        if (default_state >= options->size() || default_state == current.option_index)
            return {};
        return {
            current.the_switch, current.type, options->GetValueAt(default_state)->str(), current.reset_value, current.switch_index, default_state,
        };
    }
    return {};
}

Switches::SwitchOption Switches::FindRadioGroupOption(an<ConfigMap> the_switch, function<FindResult(SwitchOption option)> callback) {
...
StringSlice Switches::GetStateLabel(an<ConfigMap> the_switch, size_t state_index, bool abbreviated) {
    if (!the_switch)
        return {nullptr, 0};
...
StringSlice Switches::GetStateLabel(const string& option_name, int state, bool abbreviated) {
    ...
    if (the_option.type == kRadioGroup) {
        ...
        return state ? GetStateLabel(the_option.the_switch, the_option.option_index, abbreviated) : StringSlice{nullptr, 0};
    }
    return {nullptr, 0};
}
