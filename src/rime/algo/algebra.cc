...
void Script::Merge(const string& s, const SpellingProperties& sp, const vector<Spelling>& v) {
...
void Script::Dump(const path& file_path) const {
    ...
    for (const value_type& v : *this) {
        ...
        for (const Spelling& s : v.second) {
            out << (first ? v.first : "") << '\t' << s.str << '\t' << "-ac?!"[s.properties.type] << '\t' << s.properties.credibility << '\t' << s.properties.tips << std::endl;
            first = false;
        }
    }
    out.close();
...
bool Projection::Load(an<ConfigList> settings) {
    ...
    for (size_t i = 0; i < settings->size(); ++i) {
        ...
        if (!x) {
            LOG(ERROR) << "Error loading spelling algebra definition #" << (i + 1) << ": '" << formula << "'.";
            success = false;
            break;
        }
