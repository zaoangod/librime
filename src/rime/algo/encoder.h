...
class PhraseCollector {
   public:
    ...
    virtual void CreateEntry(const string& phrase, const string& code_str, const string& value) = 0;
...
class RIME_DLL TableEncoder : public Encoder {
   public:
    ...
    const vector<TableEncodingRule>& encoding_rules() const { return encoding_rules_; }
