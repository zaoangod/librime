...
void RawCode::FromString(const string& code_str) {
    *dynamic_cast<vector<string>*>(this) = strings::split(code_str, " ", strings::SplitBehavior::SkipToken);
}

TableEncoder::TableEncoder(PhraseCollector* collector) : Encoder(collector), loaded_(false), max_phrase_length_(0) {}
    ...
    if (auto rules = config->GetList("encoder/rules")) {
        for (auto it = rules->begin(); it != rules->end(); ++it) {
            ...
            if (an<ConfigValue> value = rule->GetValue("length_equal")) {
                ...
                if (range->size() != 2 || !range->GetValueAt(0) || !range->GetValueAt(1) || !range->GetValueAt(0)->GetInt(&r.min_word_length) || !range->GetValueAt(1)->GetInt(&r.max_word_length) || r.min_word_length > r.max_word_length) {
...
bool TableEncoder::Encode(const RawCode& code, string* result) {
    ...
    for (const TableEncodingRule& rule : encoding_rules_) {
        ...
        for (const CodeCoords& current : rule.coords) {
            ...
            if ((current.char_index < 0 || current.code_index < 0) && c.char_index == encoded.char_index && c.code_index <= encoded.code_index && (current.char_index != previous.char_index || current.code_index != previous.code_index)) {
...
int TableEncoder::CalculateCodeIndex(const string& code, int index, int start) {
    DLOG(INFO) << "code = " << code << ", index = " << index << ", start = " << start;
    // tail_anchor = '|'
...
bool TableEncoder::DfsEncode(const string& phrase, const string& value, size_t start_pos, RawCode* code, int* limit) {
...
bool ScriptEncoder::DfsEncode(const string& phrase, const string& value, size_t start_pos, RawCode* code, int* limit) {
    if (start_pos == phrase.length()) {
        if (limit) {
            --*limit;
        }
