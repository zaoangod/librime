...
using VertexQueue = std::priority_queue<Vertex, vector<Vertex>, std::greater<Vertex>>;

const double kCompletionPenalty = -0.6931471805599453;     // log(0.5)
const double kCorrectionCredibility = -4.605170185988091;  // log(0.01)

int Syllabifier::BuildSyllableGraph(const string& input, Prism& prism, SyllableGraph* graph) {
    if (input.empty())
        return 0;
    ...
    while (!queue.empty()) {
        ...
        if (!matches.empty()) {
            ...
            for (const auto& m : matches) {
                ...
                while (end_pos < input.length() && delimiters_.find(input[end_pos]) != string::npos)
                    ++end_pos;
                ...
                while (!accessor.exhausted()) {
                    ...
                    if (strict_spelling_ && matches_input && props.type != kNormalSpelling) {
                        ...
                        if (corrector_ && exact_match_syllables.find(m.value) == exact_match_syllables.end()) {
    ...
    SpellingType last_type = (std::max)(graph->vertices[farthest], kFuzzySpelling);
    ...
    if (enable_completion_ && farthest < input.length()) {
        ...
        if (!keys.empty()) {
            ...
            if (spellings.empty()) {
                ...
                DLOG(INFO) << "added to syllable graph, completion: [" << current_pos << ", " << end_pos << ")";
                farthest = end_pos;
            }
        }
    }
