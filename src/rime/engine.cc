...
ConcreteEngine::ConcreteEngine() {
    ...
    context_->option_update_notifier().connect([this](Context* ctx, const string& option) { OnOptionUpdate(ctx, option); });
    context_->property_update_notifier().connect([this](Context* ctx, const string& property) { OnPropertyUpdate(ctx, property); });
...
void ConcreteEngine::Compose(Context* ctx) {
    ...
    if (ctx->caret_pos() < ctx->input().length() && ctx->caret_pos() == comp.GetConfirmedPosition()) {
...
void ConcreteEngine::CalculateSegmentation(Segmentation* segments) {
    DLOG(INFO) << "CalculateSegmentation, segments: " << segments->size() << ", finished? " << segments->HasFinishedSegmentation();
...
void ConcreteEngine::TranslateSegments(Segmentation* segments) {
    ...
    for (Segment& segment : *segments) {
        DLOG(INFO) << "segment [" << segment.start << ", " << segment.end << "), status: " << segment.status;
...
template <typename T>
inline void CreateComponentsFromList(Engine* engine, Config* config, const string& config_key, const string& component_type, vector<an<T>>& target_collection) {
    if (auto component_list = config->GetList(config_key)) {
        ...
        for (size_t i = 0; i < n; ++i) {
            ...
            if (!component) {
                LOG(ERROR) << "error creating " << component_type << " from ticket: '" << ticket.klass << "'";
                continue;
            }
...
void ConcreteEngine::InitializeOptions() {
    ...
    switches.FindOption([this](Switches::SwitchOption option) {
        LOG(INFO) << "found switch option: " << option.option_name << ", reset: " << option.reset_value;
        if (option.reset_value >= 0) {
            if (option.type == Switches::kToggleOption) {
                ...
                context_->set_option(option.option_name, static_cast<int>(option.option_index) == option.reset_value);
            }
        }
