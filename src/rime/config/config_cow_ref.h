...
class ConfigCowRef : public ConfigItemRef {
   public:
    ConfigCowRef(an<ConfigItemRef> parent, string key) : ConfigItemRef(nullptr), parent_(parent), key_(key) {}
...
template <>
inline an<ConfigItem> ConfigCowRef<ConfigMap>::Read(const an<ConfigMap>& map, const string& key) {
    return map->Get(key);
}

template <>
inline void ConfigCowRef<ConfigMap>::Write(const an<ConfigMap>& map, const string& key, an<ConfigItem> value) {
    map->Set(key, value);
}

template <>
inline an<ConfigItem> ConfigCowRef<ConfigList>::Read(const an<ConfigList>& list, const string& key) {
    return list->GetAt(ConfigData::ResolveListIndex(list, key, true));
}

template <>
inline void ConfigCowRef<ConfigList>::Write(const an<ConfigList>& list, const string& key, an<ConfigItem> value) {
