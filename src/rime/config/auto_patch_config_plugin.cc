...
static string remove_suffix(const string& input, const string& suffix) {
    return boost::ends_with(input, suffix) ? input.substr(0, input.length() - suffix.length()) : input;
}

// auto-patch applies to all loaded config resources, including dependencies.
// therefore it's done at the end of Compile phase.
bool AutoPatchConfigPlugin::ReviewCompileOutput(ConfigCompiler* compiler, an<ConfigResource> resource) {
    ...
    LOG(INFO) << "auto-patch " << resource->resource_id << ":/__patch: " << patch_resource_id << ":/patch?";
...
bool AutoPatchConfigPlugin::ReviewLinkOutput(ConfigCompiler* compiler, an<ConfigResource> resource) {
    return true;
}
