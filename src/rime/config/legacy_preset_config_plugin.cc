...
bool LegacyPresetConfigPlugin::ReviewCompileOutput(ConfigCompiler* compiler, an<ConfigResource> resource) {
    return true;
}

bool LegacyPresetConfigPlugin::ReviewLinkOutput(ConfigCompiler* compiler, an<ConfigResource> resource) {
    ...
    if (auto preset = resource->data->Traverse("punctuator/import_preset")) {
        ...
        if (!IncludeReference{reference}.TargetedAt(Cow(resource, "punctuator")).Resolve(compiler)) {
            LOG(ERROR) << "failed to include section " << reference;
            return false;
        }
    }
    if (auto preset = resource->data->Traverse("recognizer/import_preset")) {
        ...
        if (!IncludeReference{reference}.TargetedAt(Cow(resource, "recognizer")).Resolve(compiler)) {
            LOG(ERROR) << "failed to include section " << reference;
            return false;
        }
    }
