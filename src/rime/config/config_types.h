...
class ConfigListEntryRef : public ConfigItemRef {
   public:
    ConfigListEntryRef(ConfigData* data, an<ConfigList> list, size_t index) : ConfigItemRef(data), list_(list), index_(index) {}
...
class ConfigMapEntryRef : public ConfigItemRef {
   public:
    ConfigMapEntryRef(ConfigData* data, an<ConfigMap> map, const string& key) : ConfigItemRef(data), map_(map), key_(key) {}
    using ConfigItemRef::operator=;
