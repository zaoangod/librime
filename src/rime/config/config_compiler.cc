...
string ConfigDependencyGraph::current_resource_id() const {
    return key_stack.empty() ? string() : boost::trim_right_copy_if(key_stack.front(), boost::is_any_of(":"));
}
...
static an<ConfigItem> ResolveReference(ConfigCompiler* compiler, const Reference& reference);
...
static bool EditNode(an<ConfigItemRef> target, const string& key, const an<ConfigItem>& value, bool merge_tree);
...
inline static bool IsAppending(const string& key) {
    return key == ConfigCompiler::APPEND_DIRECTIVE || boost::ends_with(key, ADD_SUFFIX_OPERATOR);
}
inline static bool IsMerging(const string& key, const an<ConfigItem>& value, bool merge_tree) {
    return key == ConfigCompiler::MERGE_DIRECTIVE || boost::ends_with(key, ADD_SUFFIX_OPERATOR) || (merge_tree && (!value || Is<ConfigMap>(value)) && !boost::ends_with(key, EQU_SUFFIX_OPERATOR));
}

inline static string StripOperator(const string& key, bool adding) {
    return (key == ConfigCompiler::APPEND_DIRECTIVE || key == ConfigCompiler::MERGE_DIRECTIVE) ? "" : boost::erase_last_copy(key, adding ? ADD_SUFFIX_OPERATOR : EQU_SUFFIX_OPERATOR);
}
...
static bool EditNode(an<ConfigItemRef> head, const string& key, const an<ConfigItem>& value, bool merge_tree) {
    ...
    DLOG(INFO) << "appending: " << appending << ", merging: " << merging << ", path: " << path;
    auto find_target_node = merge_tree ? &TypeCheckedCopyOnWrite : &TraverseCopyOnWrite;
    ...
    if ((appending || merging) && **target) {
        ...
        return !value ||  // no-op
               (appending && (AppendToString(target, As<ConfigValue>(value)) || AppendToList(target, As<ConfigList>(value)))) || (merging && MergeTree(target, As<ConfigMap>(value)));
    } else {
...
static void InsertByPriority(vector<of<Dependency>>& list, const an<Dependency>& value) {
    auto upper = std::upper_bound(list.begin(), list.end(), value, [](const an<Dependency>& lhs, const an<Dependency>& rhs) { return lhs->priority() < rhs->priority(); });
    list.insert(upper, value);
}

void ConfigDependencyGraph::Add(an<Dependency> dependency) {
    DLOG(INFO) << "ConfigDependencyGraph::Add(), node_stack.size() = " << node_stack.size();
    if (node_stack.empty())
        return;
    ...
    DLOG(INFO) << "target_path = " << target_path << ", #deps = " << target_deps.size();
    ...
    for (auto child = node_stack.rbegin(); child != node_stack.rend(); ++child) {
        ...
        InsertByPriority(parent_deps, New<PendingChild>(parent_path + "/" + last_key, *child));
        DLOG(INFO) << "parent_path = " << parent_path << ", #deps = " << parent_deps.size();
...
ConfigCompiler::ConfigCompiler(ResourceResolver* resource_resolver, ConfigCompilerPlugin* plugin) : resource_resolver_(resource_resolver), plugin_(plugin), graph_(new ConfigDependencyGraph) {}
...
Reference ConfigCompiler::CreateReference(const string& qualified_path) {
    ...
    string resource_id = resource_resolver_->ToResourceId((separator == string::npos || separator == 0) ? graph_->current_resource_id() : qualified_path.substr(0, separator));
    string local_path = (separator == string::npos) ? qualified_path.substr(0, end) : qualified_path.substr(separator + 1, optional ? end - separator - 1 : end);
...
void ConfigCompiler::Push(an<ConfigList> config_list, size_t index) {
    graph_->Push(New<ConfigListEntryRef>(nullptr, config_list, index), ConfigData::FormatListIndex(index));
}
...
void ConfigCompiler::EnumerateResources(function<void(an<ConfigResource> resource)> process_resource) {
    for (const auto& r : graph_->resources) {
        process_resource(r.second);
    }
}

an<ConfigResource> ConfigCompiler::GetCompiledResource(const string& resource_id) const {
...
an<ConfigResource> ConfigCompiler::Compile(const string& file_name) {
    ...
    Push(resource);
    resource->loaded = resource->data->LoadFromFile(resource_resolver_->ResolvePath(resource_id), this);
    Pop();
    if (plugin_)
...
static bool ResolveBlockingDependencies(ConfigCompiler* compiler, const string& path) {
...
static an<ConfigItem> GetResolvedItem(ConfigCompiler* compiler, an<ConfigResource> resource, const string& path) {
    ...
    for (const auto& key : keys) {
        if (!ResolveBlockingDependencies(compiler, node_path)) {
            LOG(WARNING) << "accessing blocking node with unresolved dependencies: " << node_path;
...
bool ConfigCompiler::blocking(const string& full_path) const {
    ...
    return found != graph_->deps.end() && !found->second.empty() && found->second.back()->blocking();
}
...
static bool ParseList(bool (*parser)(ConfigCompiler*, const an<ConfigItem>&), ConfigCompiler* compiler, const an<ConfigItem>& item) {
    if (Is<ConfigList>(item)) {
...
static bool ParsePatch(ConfigCompiler* compiler, const an<ConfigItem>& item) {
    if (Is<ConfigValue>(item)) {
        ...
        compiler->AddDependency(New<PatchReference>(compiler->CreateReference(path)));
        return true;
    }
    if (Is<ConfigMap>(item)) {
...
bool ConfigCompiler::Link(an<ConfigResource> target) {
    ...
    return ResolveDependencies(found->first + ":") && (plugin_ ? plugin_->ReviewLinkOutput(this, target) : true);
}

static bool HasCircularDependencies(ConfigDependencyGraph* graph, const string& path) {
    for (const auto& x : graph->resolve_chain) {
        if (boost::starts_with(x, path) && (x.length() == path.length() || x[path.length()] == '/'))
            return true;
    }
    return false;
}
