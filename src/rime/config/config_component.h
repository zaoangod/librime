...
struct ConfigResourceProvider {
    ...
    RIME_DLL static ResourceResolver* CreateResourceResolver(const ResourceType& resource_type);
};
...
class ConfigComponent : public ConfigComponentBase {
   public:
    ...
    ConfigComponent(function<void(Loader* loader)> setup) : ConfigComponentBase(ResourceProvider::CreateResourceResolver(ResourceProvider::kDefaultResourceType)) { setup(&loader_); }

   private:
    an<ConfigData> LoadConfig(const string& config_id) override { return loader_.LoadConfig(resource_resolver_.get(), config_id); }
    Loader loader_;
};

class ConfigLoader {
   public:
    RIME_DLL an<ConfigData> LoadConfig(ResourceResolver* resource_resolver, const string& config_id);
