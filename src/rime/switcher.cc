...
static an<ConfigValue> ParseSchemaListEntry(Config* config, an<ConfigMap> entry_map) {
    if (!entry_map)
        return nullptr;
    ...
    if (auto case_conditions = As<ConfigList>(entry_map->Get("case"))) {
        for (auto iter = case_conditions->begin(); iter != case_conditions->end(); ++iter) {
            if (auto condition_variable = As<ConfigValue>(*iter)) {
                bool condition_met = false;
                if (!config->GetBool(condition_variable->str(), &condition_met) || !condition_met) {
                    return nullptr;
                }
            }
        }
...
int Switcher::ForEachSchemaListEntry(Config* config, function<bool(const string& schema_id)> process_entry) {
