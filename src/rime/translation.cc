...
int Translation::Compare(an<Translation> other, const CandidateList& candidates) {
...
MergedTranslation::MergedTranslation(const CandidateList& candidates) : previous_candidates_(candidates) {
    set_exhausted(true);
}
...
CacheTranslation::CacheTranslation(an<Translation> translation) : translation_(translation) {
...
DistinctTranslation::DistinctTranslation(an<Translation> translation) : CacheTranslation(translation) {}
...
bool DistinctTranslation::Next() {
    ...
    } while (!exhausted() && AlreadyHas(Peek()->text()));  // skip duplicate candidates
    return true;
}
...
PrefetchTranslation::PrefetchTranslation(an<Translation> translation) : translation_(translation) {
    set_exhausted(!translation_ || translation_->exhausted());
}
