...
string ResourceResolver::ToFilePath(const string& resource_id) const {
    bool missing_prefix = !path(resource_id).has_parent_path() && !boost::starts_with(resource_id, type_.prefix);
    bool missing_suffix = !boost::ends_with(resource_id, type_.suffix);
    return (missing_prefix ? type_.prefix : "") + resource_id + (missing_suffix ? type_.suffix : "");
}

path ResourceResolver::ResolvePath(const string& resource_id) {
    return std::filesystem::absolute(root_path_ / (type_.prefix + resource_id + type_.suffix));
}
...
path FallbackResourceResolver::ResolvePath(const string& resource_id) {
    ...
    if (!std::filesystem::exists(default_path)) {
        auto fallback_path = std::filesystem::absolute(fallback_root_path_ / (type_.prefix + resource_id + type_.suffix));
