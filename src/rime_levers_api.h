...
typedef struct RIME_FLAVORED(rime_levers_api_t) {
    ...
    RimeCustomSettings* (*custom_settings_init)(const char* config_id, const char* generator_id);
    ...
    Bool (*get_available_schema_list)(RimeSwitcherSettings* settings, RimeSchemaList* list);
    Bool (*get_selected_schema_list)(RimeSwitcherSettings* settings, RimeSchemaList* list);
    ...
    Bool (*select_schemas)(RimeSwitcherSettings* settings, const char* schema_id_list[], int count);
    ...
    Bool (*customize_item)(RimeCustomSettings* settings, const char* key, RimeConfig* value);
