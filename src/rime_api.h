...
#define RIME_PROVIDED(p, member) ((p) && RIME_STRUCT_HAS_MEMBER(*(p), (p)->member) && (p)->member)
...
typedef struct RIME_FLAVORED(rime_api_t) {
    ...
    Bool (*get_context)(RimeSessionId session_id, RIME_FLAVORED(RimeContext) * context);
    Bool (*free_context)(RIME_FLAVORED(RimeContext) * ctx);
    Bool (*get_status)(RimeSessionId session_id, RIME_FLAVORED(RimeStatus) * status);
